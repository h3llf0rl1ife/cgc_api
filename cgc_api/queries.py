...
class Queries(object):
    ...
    def validateDate(kwarg, default=None):
        ...
        try:
            ...
            dates = (datetime.datetime(1900, 1, 1, 00, 00, 00),
                     datetime.datetime.now())
            if default:
                return dates[default].strftime('%Y-%m-%d %H:%M:%S')
        return default

    def is_op_auth_for_tache(self, args):
        query = '''
            SELECT
        ...
        if kwargs['pcodeOp'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
            kwargs['OPTIONAL_ARG_2'] = kwargs['OPTIONAL_ARG_2'][4:]

        if kwargs['pCodeTache'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''

        if kwargs['OPTIONAL_ARG_2'] == kwargs['OPTIONAL_ARG_1'] \
                and kwargs['OPTIONAL_ARG_2'] == '':
            query = query.replace('WHERE', '')

        return query.format(**kwargs).format(**kwargs)

    def Param_ls_clients_gp(self, args):
        query = '''
            SELECT
        ...
        if kwargs['Param_gp'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''

        if kwargs['Param_not_in'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''
    ...
    def Param_supp_objectif_secteurs(self, args):
        query = '''
            DELETE o FROM
                T_OBJECTIF_SECTEURS o
                {JOINS}
            WHERE
                o.DATE_OBJECTIF = '{Param_date_journee}'
                {OPTIONAL_ARG_1}
        '''
        ...
        if kwargs['Param_code_secteur'] in self.null_values:
            kwargs['JOINS'] = '''INNER JOIN T_SECTEUR s on s.CODE_SECTEUR = o.CODE_SECTEUR
    ...
    def Req_affectation_chargement(self, args):
        query = '''
            SELECT
            ...
            FROM
                T_CHARGEMENT
            WHERE
                T_CHARGEMENT.CODE_AGCE = {CODE_AGCE}
                {OPTIONAL_ARG_1}
                {OPTIONAL_ARG_2}
        '''
        ...
        kwargs['OPTIONAL_ARG_1'] = '''AND T_CHARGEMENT.DATE_CHARGEMENT = '{Param_date_chargement}' '''
        kwargs['OPTIONAL_ARG_2'] = 'AND	T_CHARGEMENT.code_secteur = {Param_code_secteur}'

        if kwargs['Param_date_chargement'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_anc_solde_dep(self, args):
        query = '''
            SELECT
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_MOUVEMENTS_CAISSE.CODE_CAISSE = {Param_code_caisse}'
        kwargs['OPTIONAL_ARG_2'] = 'AND T_OPERATIONS_CAISSE.DATE_OPERATION < \'{Param_date_journee}\''
        kwargs['OPTIONAL_ARG_3'] = 'T_OPERATIONS_CAISSE.DATE_VALIDATION = \'{Param_date_journee}\' OR'

        if kwargs['Param_code_caisse'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_annulation_facture(self, args):
        ...
        if kwargs['Param_num_facture'] in self.null_values:
            return ValueError
    ...
    def Req_annulation_gratuit_prevente(self, args):
        query = '''
            ...
            WHERE
                ID_COMMANDE IN (
                    SELECT ID_COMMANDE
                    FROM T_COMMANDE_CLIENT
                    WHERE DATE_COMMANDE='{param_date}'
                        AND code_secteur={param_code_secteur})
                AND T_LIGNE_COMMANDE.QTE_LIVREE = 0
                AND T_LIGNE_COMMANDE.QTE_COMMANDE>0
        '''
    ...
    def Req_article_livraison(self, args):
        query = '''
            SELECT
            ...
            FROM
                ...
                T_OPERATIONS
            WHERE
                T_MOUVEMENTS.ORIGINE = T_OPERATIONS.CODE_OPERATION
        ...
        if kwargs['Param_origine'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''

        if kwargs['Param_code_article'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''

        return query.format(**kwargs).format(**kwargs)

    def Req_article_livree_gms_date(self, args):
        query = '''
            SELECT
        ...
        if kwargs['Param_code_client'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''

        if kwargs['Param_date_livraison'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''

        return query.format(**kwargs).format(**kwargs)

    def Req_article_magasins(self, args):
        query = '''
            SELECT
    ...
    def Req_articles_charges(self, args):
        query = '''
            SELECT
            ...
            FROM
                ...
                T_CHARGEMENT
            WHERE
                T_PRODUITS_CHARGEE.CODE_CHARGEMENT = T_CHARGEMENT.CODE_CHARGEMENT
                {OPTIONAL_ARG_1}
                {OPTIONAL_ARG_2}
                {OPTIONAL_ARG_3}
        ...
        kwargs['OPTIONAL_ARG_2'] = 'AND T_CHARGEMENT.code_secteur = {Param_code_secteur}'
        kwargs['OPTIONAL_ARG_3'] = 'AND T_CHARGEMENT.DATE_CHARGEMENT = \'{Param_date_chargement}\''
    ...
    def Req_articles_cmd(self, args):
        query = '''
            SELECT 
                ...
                T_ARTICLES_AGENCE.TYPE_CAISSE AS TYPE_CAISSE,	
                T_ARTICLES_AGENCE.TYPE_PALETTE AS TYPE_PALETTE
            FROM 
                T_ARTICLES,
                T_ARTICLES_AGENCE,
                T_PRIX,	
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_PRODUITS_COMMANDES.ID_COMMANDE = {Param_id_commande}'
        kwargs['OPTIONAL_ARG_3'] = 'AND T_PRIX.Date_Debut <= \'{param_dt}\''
        kwargs['OPTIONAL_ARG_4'] = 'AND T_PRIX.Date_Fin >= \'{param_dt}\''

        if kwargs['Param_id_commande'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_articles_enseigne(self, args):
        query = '''
            SELECT 
        ...
        if kwargs['Param_ID_ENSEIGNE'] in self.null_values:
            kwargs['CODE_BLOCK_1'] = ''
    ...
    def Req_articles_livraison_client(self, args):
        ...
        if kwargs['CODE_AGCE'] in self.null_values:
            return ValueError

        kwargs['Param_date_livraison'] = self.validateDate(kwargs['Param_date_livraison'])
        
        kwargs['OPTIONAL_ARG_1'] = 'AND T_LIVRAISON.DATE_LIVRAISON = \'{Param_date_livraison}\''
        kwargs['OPTIONAL_ARG_2'] = 'AND T_PRODUITS_LIVREES.TYPE_MVT = \'{Param_type_mvt}\''
        kwargs['OPTIONAL_ARG_3'] = 'AND T_LIVRAISON.CODE_CLIENT = {Param_code_client}'

        if kwargs['Param_date_livraison'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''

        if kwargs['Param_type_mvt'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''
    ...
    def Req_articles_livraison_secteur(self, args):
        query = '''
            ...
            WHERE 
                T_LIVRAISON.NUM_LIVRAISON = T_PRODUITS_LIVREES.NUM_LIVRAISON
                AND T_PRODUITS_LIVREES.STATUT <> 'A'
                {OPTIONAL_ARG_1}
                {OPTIONAL_ARG_2}
                AND T_LIVRAISON.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def req_autorisation_caisserie(self, args):
        query = '''
            SELECT 
                a.ID_JUSTIFICATION AS ID_JUSTIFICATION,	
                a.DATE_HEURE AS DATE_HEURE
            FROM 
                T_AUTORISATION_SOLDE_CAISSERIE a
                INNER JOIN dbo.T_OPERATEUR o
                    ON o.CODE_OPERATEUR = a.CODE_OPERATEUR
            WHERE 
                a.DATE_HEURE = '{Param_dt}'
                AND o.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_bl_non_envoyer(self, args):
        query = '''
            SELECT 
                T_LIVRAISON.NUM_LIVRAISON AS NUM_LIVRAISON,	
    ...
    def Req_bordereau(self, args):
        ...
        if kwargs['Param_type'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_budget_mensuel(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_BUDGET_MENSUEL.CODE_AGCE = {CODE_AGCE}
                {OPTIONAL_ARG_1}
        '''
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_BUDGET_MENSUEL.DATE_BUDGET = \'{Param_date_budget}\''

        if kwargs['Param_date_budget'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_ca_secteur_periode(self, args):
        query = '''
            SELECT 
                T_CHARGEMENT.CODE_SECTEUR AS code_secteur,	
    ...
    def Req_chargement(self, args):
        query = '''
            SELECT 
                ...
                T_ARTICLES_AGENCE.CONDITIONNEMENT AS CONDITIONNEMENT,	
                T_ARTICLES_AGENCE.QTE_PALETTE AS QTE_PALETTE,	
                T_ARTICLES_AGENCE.ACTIF AS ACTIF,	
                T_PRODUITS_CHARGEE.QTE_CHARGEE AS QTE_CHARGEE,	
                T_PRODUITS_CHARGEE.QTE_COND AS QTE_COND,	
                T_PRODUITS_CHARGEE.QTE_CHARGEE_SUPP AS QTE_CHARGEE_SUPP,	
                T_PRODUITS_CHARGEE.QTE_ECART AS QTE_ECART,	
        ...
        if kwargs['Param_code_chargement'] in self.null_values:
            return ValueError

        return query.format(**kwargs)

    def Req_chargement_article(self, args):
        query = '''
            SELECT 
                T_CHARGEMENT.code_vendeur AS code_vendeur,	
    ...
    def Req_chargement_cond(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_COND_CHARGEE.CODE_CHARGEMENT = {Param_code_chargement}
        '''
    ...
    def Req_chargement_non_valide(self, args):
        query = '''
            SELECT 
                T_CHARGEMENT.DATE_CHARGEMENT AS DATE_CHARGEMENT,	
    ...
    def Req_chargement_par_article(self, args):
        query = '''
            SELECT 
                T_ARTICLES.CODE_ARTICLE AS CODE_ARTICLE,	
                T_PRODUITS_CHARGEE.CODE_CHARGEMENT AS CODE_CHARGEMENT,	
    ...
    def Req_chargement_par_produit(self, args):
        ...
        if kwargs['Param_code_produit'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''
    ...
    def Req_chargement_secteur(self, args):
        query = '''
            SELECT 
                T_PRODUITS_CHARGEE.CODE_CHARGEMENT AS CODE_CHARGEMENT,	
                T_CHARGEMENT.DATE_CHARGEMENT AS DATE_CHARGEMENT,	
    ...
    def Req_cheque_non_envoyer(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_DT_DECOMPTE.MOTIF_ENVOI <> 1
                AND T_DECOMPTE.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_clients_n1(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_MOY_VENTE_CLIENTS.DATE_VENTE = '{Param_date}'
                AND T_CLIENTS.CODE_AGCE = {CODE_AGCE}
        '''
        ...
        if kwargs['Param_date'] in self.null_values:
            return ValueError

        return query.format(**kwargs)

    def Req_code_chargement(self, args):
        query = '''
            SELECT 
                T_CHARGEMENT.CODE_CHARGEMENT AS CODE_CHARGEMENT,	
                T_CHARGEMENT.DATE_CHARGEMENT AS DATE_CHARGEMENT,	
    ...
    def Req_commande_gms_date(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_COMMANDES.CODE_CLIENT = {Param_code_client}'
    ...
    def Req_commande_secteur_produit(self, args):
        query = '''
            SELECT 
                T_COMMANDES.DATE_LIVRAISON AS DATE_LIVRAISON,	
                T_COMMANDES.code_secteur AS code_secteur,	
    ...
    def Req_cond_charge_operateur(self, args):
        query = '''
            ...
            FROM 
                T_COND_CHARGEE
                INNER JOIN T_CHARGEMENT
                  ON T_COND_CHARGEE.CODE_CHARGEMENT = T_CHARGEMENT.CODE_CHARGEMENT
                    AND T_CHARGEMENT.CODE_AGCE = {CODE_AGCE}
            WHERE
        ...
        if kwargs['Param_date_mvt'] in self.null_values:
            return ValueError

        kwargs['OPTIONAL_ARG_1'] = 'AND T_COND_CHARGEE.CODE_COND = {Param_code_cond}'

        if kwargs['Param_code_cond'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_cond_chargee(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_COND_CHARGEE.code_secteur = {Param_code_secteur}'
        kwargs['OPTIONAL_ARG_2'] = 'AND T_COND_CHARGEE.CODE_COND = {Paramcode_cp}'

        if kwargs['Param_code_secteur'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''

        if kwargs['Paramcode_cp'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''
    ...
    def req_conseigne_deconseige(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_REGELEMENT_COND.CODE_AGCE = {CODE_AGCE}
            GROUP BY 
    ...
    def Req_contrubition_canal(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND STATISTIQUES.dbo.T_STATISTIQUES.DATE_JOURNEE BETWEEN '{Param_dt1}' AND '{Param_dt2}'
    ...
    def Req_date_distribution_remise(self, args):
        query = '''
            SELECT 
                T_FACTURE.CODE_CLIENT AS CODE_CLIENT,	
        ...
        if kwargs['Param_code_superviseur'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''

        if kwargs['Param_resp_vente'] in self.null_values:
            kwargs['OPTIONAL_ARG_3'] = ''
    ...
    def Req_dernier_maj_stock(self, args):
        query = '''
            SELECT 
        ...
        if kwargs['Param_cde_secteur'] in self.null_values:
            return ValueError
    ...
    def Req_dernier_rib(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_DECOMPTE.CODE_CLIENT = {Param_code_client}'
        kwargs['OPTIONAL_ARG_2'] = 'AND	T_DT_DECOMPTE.GP_CLIENT = {Param_gp_client}'

        if kwargs['Param_code_client'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''

        if kwargs['Param_gp_client'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''

        return query.format(**kwargs).format(**kwargs)

    def Req_det_borderau_valeurs(self, args):
        query = '''
            SELECT 
            ...
            WHERE 
                ...
                AND T_DT_BORDEREAU.ID_BORDEREAU = {Param_id_bordereau}
                AND T_DECOMPTE.CODE_AGCE = {CODE_AGCE}
        '''
        ...
        if kwargs['Param_id_bordereau'] in self.null_values:
            return ValueError
    ...
    def Req_dt_articles(self, args):
        query = '''
            SELECT 
                ...
                T_ARTICLES_AGENCE.QTE_PACK AS QTE_PACK,	
                T_ARTICLES_AGENCE.QTE_PALETTE AS QTE_PALETTE,	
            ...
            WHERE 
                ...
                AND	T_ARTICLES.CODE_ARTICLE = T_ARTICLES_AGENCE.CODE_ARTICLE
                AND T_PRIX.CODE_AGCE = {Param_code_agence}
                AND T_ARTICLES_AGENCE.CODE_AGCE = {CODE_AGCE}
    ...
    def Req_dt_courrier(self, args):
        ...
        if kwargs['Param_id_envoi'] in self.null_values:
            return ValueError
    ...
    def Req_dt_mouvement_cond(self, args):
        query = '''
            ...
            WHERE 
				...
				AND T_MOUVEMENTS_CAISSERIE.CODE_MAGASIN = T_MAGASINS.CODE_MAGASIN
                {OPTIONAL_ARG_1}
                AND T_MAGASINS.CODE_AGCE = {CODE_AGCE}
        '''
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_MOUVEMENTS_CAISSERIE.ORIGINE = {Param_origine}'
    ...
    def Req_dt_operation(self, args):
        ...
        if kwargs['Paramtype_produit'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''
    ...
    def Req_dt_prelevement(self, args):
        query = '''
            SELECT 
                ...
                T_DT_PRELEVEMENT_COND.SOLDE_CS2 AS SOLDE_CS2,
                T_DT_PRELEVEMENT_COND.DECON_CS2 AS DECON_CS2
            FROM 
                T_FONCTION,	
                T_OPERATEUR,	
            ...
            WHERE 
                ...
                AND T_DT_PRELEVEMENT_COND.ID_PRELEVEMENT = {Param_id_prelevement}
        ...
        if kwargs['Param_id_prelevement'] in self.null_values:
            return ValueError
    ...
    def Req_dt_reclamation(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_DT_RECLAMATION.ID_RECLAMATION = {Param_id_reclamation}
        '''
        ...
        if kwargs['Param_id_reclamation'] in self.null_values:
            return ValueError
    ...
    def Req_dt_rendus(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_LIGNE_RETOUR_RENDUS.ID_RETOUR = {Param_id_retour}
        '''
        ...
        if kwargs['Param_id_retour'] in self.null_values:
            return ValueError

        return query.format(**kwargs)

    def Req_dt_retour(self, args):
        query = '''
            SELECT 
            ...
            WHERE 
                ...
                AND T_LIGNE_RETOUR_CAISSERIE.ID_RETOUR = {Param_id_retour}
            ORDER BY 
                RANG ASC
        '''
    ...
    def Req_ecarts(self, args):
        ...
        if kwargs['Param_diff_categorie'] in self.null_values:
            return ValueError
        
        kwargs['OPTIONAL_ARG_1'] = 'AND T_RETOURS_USINE.VALID = {Param_valid}'
        kwargs['OPTIONAL_ARG_2'] = 'AND T_RETOURS_USINE.CATEGORIE = \'{Param_egal_categorie}\''

        if kwargs['Param_valid'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''

        if kwargs['Param_egal_categorie'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''
    ...
    def Req_ecarts_caisserie(self, args):
        ...
        for kwarg in ('Param_egal_categorie', 'CODE_AGCE'):
    ...
    def Req_ecarts_inventaire_par_magasin(self, args):
        query = '''
            SELECT 
    ...
    def req_etat_chargement(self, args):
        query = '''
            SELECT 
                T_CHARGEMENT.DATE_CHARGEMENT AS DATE_CHARGEMENT,	
                T_CHARGEMENT.code_secteur AS code_secteur,	
    ...
    def Req_etat_validation_remise(self, args):
        query = '''
            SELECT 
                T_REMISE_CLIENT_TRAD.Date_Debut AS Date_Debut,	
                T_REMISE_CLIENT_TRAD.STATUT AS STATUT
            FROM 
                T_REMISE_CLIENT_TRAD
            WHERE 
                T_REMISE_CLIENT_TRAD.Date_Debut = '{Param_DATE_DEBUT}'
                AND T_REMISE_CLIENT_TRAD.CODE_AGCE = {CODE_AGCE}
        '''
        ...
        if kwargs['Param_DATE_DEBUT'] in self.null_values:
            return ValueError

        return query.format(**kwargs)

    def Req_export_invendu_rendus(self, args):
        query = '''
            SELECT 
                T_ARTICLES.CODE_ARTICLE AS CODE_ARTICLE,	
                T_ARTICLES.RANG AS RANG,	
            ...
            WHERE 
                T_ARTICLES.CODE_ARTICLE = T_ARTICLES_AGENCE.CODE_ARTICLE
                AND T_ARTICLES_AGENCE.ACTIF = 1
                AND T_PRODUITS_CHARGEE.CODE_CHARGEMENT = T_CHARGEMENT.CODE_CHARGEMENT
                {OPTIONAL_ARG_1}
                AND T_CHARGEMENT.CODE_AGCE = {CODE_AGCE}
        ...
        kwargs['Param_date'] = self.validateDate(kwargs['Param_date'])
        kwargs['OPTIONAL_ARG_1'] = 'AND T_CHARGEMENT.DATE_CHARGEMENT = \'{Param_date}\''
    ...
    def Req_famille_gamme(self, args):
        ...
        if kwargs['Param_code_gamme'] in self.null_values:
            return ValueError
    ...
    def Req_get_prevendeur_date(self, args):
        query = '''
            ...
            FROM 
                T_COMMANDE_CLIENT
                INNER JOIN T_CLIENTS
                  ON T_COMMANDE_CLIENT.CODE_CLIENT = T_CLIENTS.CODE_CLIENT
                    AND T_CLIENTS.CODE_AGCE = {CODE_AGCE}
            WHERE 
        ...
        if kwargs['Param_DATE_COMMANDE'] in self.null_values:
            return ValueError
    ...
    def REQ_GetEnseigne(self, args):
        query = '''
            SELECT DISTINCT 
                T_GROUP_CLIENT.ID_GROUP AS ID_GP_CLIENT,	
    ...
    def Req_info_bl_mission(self, args):
        ...
        if kwargs['Param_num_bl'] in self.null_values:
            return ValueError
    ...
    def Req_info_caisse(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_CAISSE.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_info_chargement(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_CHARGEMENT.CODE_CHARGEMENT = {Param_code_chargement}
                AND T_CHARGEMENT.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_info_client(self, args):
        query = '''
            ...
            WHERE 
                T_SOUS_SECTEUR.CODE_SOUS_SECTEUR = T_CLIENTS.SOUS_SECTEUR
                AND T_CLIENTS.CODE_CLIENT = {Param_code_client}
                AND T_CLIENTS.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_info_journee(self, args):
        query = '''
            SELECT 
                T_JOURNEE.DATE_JOURNEE AS DATE_JOURNEE,	
                T_JOURNEE.CODE_AGCE AS CODE_AGCE,	
    ...
    """def Req_info_mission(self, args):
        ...
        if kwargs['Param_id_mission'] in self.null_values:
            return ValueError
    ...
    def Req_info_operateur(self, args):
        ...
        if kwargs['Param_code_operateur'] in self.null_values:
            return ValueError
    ...
    """def Req_info_trajet(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = '' if kwargs['Param_arrivee'] in self.null_values else kwargs['OPTIONAL_ARG_1']
    ...
    def Req_itineraire(self, args):
        ...
        if kwargs['Param_code_tournee'] in self.null_values:
            return ValueError

        return query.format(**kwargs)

    def Req_jours_non_clos(self, args):
        query = '''
            SELECT 
        ...
        if kwargs['Param_code_agence'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_lignes_commande(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_LIGNE_COMMANDE.ID_COMMANDE = {Param_id_commande}
            ORDER BY 
                RANG ASC
        '''
    ...
    def Req_livraison_articles(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_PRODUITS_LIVREES.CODE_ARTICLE = {Param_code_article}'
    ...
    def Req_livraison_global_client(self, args):
        query = '''
            SELECT 
                T_LIVRAISON.CODE_CLIENT AS CODE_CLIENT,	
                T_LIVRAISON.DATE_LIVRAISON AS DATE_LIVRAISON,	
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_LIVRAISON.DATE_LIVRAISON = \'{Param_date_livraison}\''
        kwargs['OPTIONAL_ARG_2'] = 'AND T_LIVRAISON.DATE_VALIDATION = \'{Param_date_validation}\''
        kwargs['OPTIONAL_ARG_3'] = 'AND T_LIVRAISON.CODE_CLIENT = {Param_code_client}'

        if kwargs['Param_date_livraison'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''

        if kwargs['Param_date_validation'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''
    ...
    def Req_livraisons_unique(self, args):
        ...
        if kwargs['Param_cat_client'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_ls_appareil(self, args):
        query = '''
            ...
            FROM 
                T_APPAREIL
            WHERE
                T_APPAREIL.CODE_AGCE = {CODE_AGCE}
                {OPTIONAL_ARG_1}
                {OPTIONAL_ARG_2}
            ORDER BY
                NUMERO_SERIE
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_APPAREIL.CODE_APPAREIL = {pAppareil}'
        if kwargs['pAppareil'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''

        kwargs['OPTIONAL_ARG_2'] = 'AND ISNULL(T_APPAREIL.CODE_OPERATEUR, -1) = {pOperateur_Moins1PourNull}'
        if kwargs['pOperateur_Moins1PourNull'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''
        
        return query.format(**kwargs).format(**kwargs)

    def Req_ls_articles(self, args):
        query = '''
            SELECT 
                T_ARTICLES.CODE_ARTICLE AS CODE_ARTICLE,	
                T_ARTICLES.LIBELLE AS LIBELLE,	
                ...
                T_FAMILLE.CODE_GAMME AS CODE_GAMME,	
                T_PRODUITS.NOM_PRODUIT AS NOM_PRODUIT,	
                T_PRODUITS.RANG_PRODUIT AS RANG_PRODUIT,	
                T_PRIX.PRIX AS PRIX_VENTE,
                T_ARTICLES.CODE_BARRE AS CODE_BARRE,	
    ...
    def Req_ls_articles_export(self, args):
        query = '''
            SELECT 
                T_ARTICLES.CODE_ARTICLE AS CODE_ARTICLE,	
                T_ARTICLES.LIBELLE AS LIBELLE,	
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_PRIX.Date_Debut <= \'{param_dt}\' AND	T_PRIX.Date_Fin >= \'{param_dt}\''
        ...
        if kwargs['Param_aff_commande'] in self.null_values:
            kwargs['OPTIONAL_ARG_3'] = ''
    ...
    def Req_ls_articles_tout(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_PRIX.CODE_AGCE = {Param_code_agce}'
        ...
        if kwargs['Param_cat'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''
    ...
    def Req_ls_autorisation_journee(self, args):
        query = '''
            ...
            FROM 
                ...
                INNER JOIN T_OPERATEUR
                  ON T_OPERATEUR.CODE_OPERATEUR = T_AUTORISATIONS_SOLDE.CODE_OPERATEUR
                    AND T_OPERATEUR.CODE_AGCE = {CODE_AGCE}
            WHERE 
    ...
    def Req_ls_autorisations(self, args):
        query = '''
            SELECT 
                T_AUTORISATIONS_SOLDE.ID_JUSTIFICATION AS ID_JUSTIFICATION,	
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_AUTORISATIONS_SOLDE.CODE_OPERATEUR = {Param_code_operateur}'
        if kwargs['Param_code_operateur'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_ls_avoirs_secteurs_caisserie(self, args):
        query = '''
            SELECT 
                T_COND_CHARGEE.DATE_CHARGEMENT AS DATE_CHARGEMENT,	
            ...
            WHERE 
                T_SECTEUR.code_secteur = T_CHARGEMENT.code_secteur
                AND	T_CHARGEMENT.CODE_CHARGEMENT = T_COND_CHARGEE.CODE_CHARGEMENT
                AND T_COND_CHARGEE.COMPTE_ECART = {Param_compte_ecart}
    ...
    def Req_ls_borderau_valeurs(self, args):
        query = '''
            SELECT 
            ...
            WHERE 
                ...
                AND T_BORDEREAU_VALEURS.ACTIF = 1
            ORDER BY 
                ID_BORDEREAU DESC
        '''
        return query
    ...
    def Req_ls_cheques(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_OPERATIONS_CAISSE.DATE_OPERATION = \'{Param_date_operation}\''
        if kwargs['Param_date_operation'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_ls_cheques_cac(self, args):
        query = '''
            ...
            FROM 
                ...
                T_DECOMPTE
            WHERE 
                T_DT_DECOMPTE.GP_CLIENT = T_GROUP_CLIENT.ID_GROUP
                AND	T_DECOMPTE.NUM_DECOMPTE = T_DT_DECOMPTE.NUM_DECOMPTE
        ...
        if kwargs['Param1'] in self.null_values:
            kwargs['Param1'] = '1900-01-01'

        if kwargs['Param2'] in self.null_values:
    ...
    def Req_ls_clients_con_dec(self, args):
        query = '''
            SELECT 
                T_CLIENTS.CODE_CLIENT AS CODE_CLIENT,	
                T_CLIENTS.CAT_CLIENT AS CAT_CLIENT,	
    ...
    def Req_ls_clients_conseigne(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = '' if kwargs['Param_param_code_secteur'] in self.null_values else kwargs['OPTIONAL_ARG_1']
        kwargs['OPTIONAL_ARG_2'] = '' if kwargs['Param_cac'] in self.null_values else kwargs['OPTIONAL_ARG_2']
        kwargs['OPTIONAL_ARG_3'] = '' if kwargs['param_not_classe'] in self.null_values else kwargs['OPTIONAL_ARG_3']
        kwargs['OPTIONAL_ARG_4'] = '' if kwargs['Param_auth_cheque'] in self.null_values else kwargs['OPTIONAL_ARG_4']
        kwargs['OPTIONAL_ARG_5'] = '' if kwargs['Param_type_pres'] in self.null_values else kwargs['OPTIONAL_ARG_5']
        kwargs['OPTIONAL_ARG_6'] = '' if kwargs['Param_actif'] in self.null_values else kwargs['OPTIONAL_ARG_6']

        return query.format(**kwargs).format(**kwargs)

    def Req_ls_clients_itineraire(self, args):
        query = '''
            SELECT 
                T_ITINERAIRES.CODE_TOURNEE AS CODE_TOURNEE,	
                T_ITINERAIRES.CODE_CLIENT AS CODE_CLIENT,	
    ...
    def Req_ls_clients_secteur(self, args):
        ...
        kwargs['OPTIONAL_ARG_2'] = '' if kwargs['Param_cac'] in self.null_values else kwargs['OPTIONAL_ARG_2']
        kwargs['OPTIONAL_ARG_3'] = '' if kwargs['Param_classe'] in self.null_values else kwargs['OPTIONAL_ARG_3']
        kwargs['OPTIONAL_ARG_4'] = '' if kwargs['Param_categorie'] in self.null_values else kwargs['OPTIONAL_ARG_4']
    ...
    def Req_ls_commande_client(self, args):
        ...
        for kwarg in ('Param_date_livraison', 'CODE_AGCE'):
            if kwargs[kwarg] in self.null_values:
                return ValueError
    ...
    def req_ls_commandes_prevente(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_COMMANDE_CLIENT.DATE_COMMANDE BETWEEN '{Param_dt1}' AND '{Param_dt2}'
        ...
        kwargs['OPTIONAL_ARG_1'] = '' if kwargs['Param_code_superviseur'] in self.null_values else kwargs['OPTIONAL_ARG_1']
        kwargs['OPTIONAL_ARG_2'] = 'AND	T_ZONE.RESP_VENTE = {Param_resp_vente}'
        kwargs['OPTIONAL_ARG_2'] = '' if kwargs['Param_resp_vente'] in self.null_values else kwargs['OPTIONAL_ARG_2']
        kwargs['OPTIONAL_ARG_3'] = 'AND	T_COMMANDE_CLIENT.CODE_CLIENT = {Param_code_client}'
        kwargs['OPTIONAL_ARG_3'] = '' if kwargs['Param_code_client'] in self.null_values else kwargs['OPTIONAL_ARG_3']
    ...
    def Req_ls_cond(self, args):
        ...
        if kwargs['Param_code_magasin'] in self.null_values:
            return ValueError
        ...
        if kwargs['Param_type_cond'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''
    ...
    def Req_ls_cond_livraison(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_COND_LIVRAISON.NUM_LIVRAISON = {Param_num_livraison}
        '''
        ...
        if kwargs['Param_num_livraison'] in self.null_values:
            return ValueError

        return query.format(**kwargs)

    def Req_ls_cond_livrees(self, args):
        query = '''
            SELECT 
                T_COND_LIVRAISON.NUM_LIVRAISON AS NUM_LIVRAISON,	
                T_COND_LIVRAISON.CODE_CP AS CODE_CP,	
    ...
    def Req_ls_courrier(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_COURRIER_AGENCE.CODE_AGCE = {CODE_AGCE}
            ORDER BY 
    ...
    def Req_ls_decompte_cheque(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND	T_BANQUES.NUM_BANQUE = T_DECOMPTE.CODE_BANQUE
                AND T_DECOMPTE.CODE_OPERATEUR = {Param_param_operateur}
                AND	T_DECOMPTE.DATE_DECOMPTE = '{Param_date_decompte}'
    ...
    def Req_ls_decompte_journee(self, args):
        query = '''
            SELECT 
                T_DECOMPTE.NUM_DECOMPTE AS NUM_DECOMPTE,	
                T_DECOMPTE.DATE_DECOMPTE AS DATE_DECOMPTE,	
        ...
        if kwargs['Param_date_decompte'] in self.null_values:
            return ValueError
    ...
    def req_ls_ecarts_controleur(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND	T_ARTICLES.CODE_ARTICLE = T_MOUVEMENTS.CODE_ARTICLE
                AND	T_PRODUITS.CODE_PRODUIT = T_ARTICLES.CODE_PRODUIT
                AND T_MOUVEMENTS.COMPTE_ECART = {Param_compte_ecart}
    ...
    def req_ls_ecarts_controleur_caisserie(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_OPERATIONS.DATE_OPERATION BETWEEN '{Param_dt1}' AND '{Param_dt2}'
    ...
    def req_ls_ecarts_journee(self, args):
        query = '''
            SELECT 
            ...
            WHERE 
                ...
                AND T_ECARTS_RENDUS.CODE_AGCE = {CODE_AGCE}
        '''
        ...
        for key in ('Param_date_situation', 'Param_dt1', 'Param_dt2'):
        ...
        if kwargs['Param_date_situation'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_ls_enseigne_secteur(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_CLIENTS.GROUP_CLIENT = T_GROUP_CLIENT.ID_GROUP
                AND T_SOUS_SECTEUR.code_secteur = {Param_CODE_SECTEUR}
                AND T_CLIENTS.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_ls_factures_clients(self, args):
        ...
        kwargs['OPTIONAL_ARG_4'] = '' if kwargs['Param_cac'] in self.null_values else kwargs['OPTIONAL_ARG_4']
        kwargs['OPTIONAL_ARG_5'] = 'AND	T_ZONE.CODE_SUPERVISEUR = {Param_code_superviseur}'
        kwargs['OPTIONAL_ARG_5'] = '' if kwargs['Param_code_superviseur'] in self.null_values else kwargs['OPTIONAL_ARG_5']
        kwargs['OPTIONAL_ARG_6'] = 'AND	T_ZONE.RESP_VENTE = {Param_resp_vente}'
        kwargs['OPTIONAL_ARG_6'] = '' if kwargs['Param_resp_vente'] in self.null_values else kwargs['OPTIONAL_ARG_6']

        return query.format(**kwargs).format(**kwargs)

    def Req_ls_gms(self, args):
        query = '''
            SELECT 
                T_CLIENTS.CODE_CLIENT AS CODE_CLIENT,	
                T_CLIENTS.CAT_CLIENT AS CAT_CLIENT,	
    ...
    def Req_ls_livraison(self, args):
        query = '''
            SELECT 
                T_LIVRAISON.NUM_LIVRAISON AS NUM_LIVRAISON,	
                T_LIVRAISON.DATE_LIVRAISON AS DATE_LIVRAISON,	
    ...
    def Req_ls_livraison_tournee_journee(self, args):
        ...
        for key in ('Param_date_livraison', 'Param_code_secteur', 'CODE_AGCE'):
            if kwargs[key] in self.null_values:
                return ValueError
    ...
    def Req_ls_livraisons_journee(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_LIVRAISON.DATE_LIVRAISON = '{Param_date_chargement}'
                AND	T_LIVRAISON.STATUT <> 'A'
    ...
    def req_ls_mvt(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_MOUVEMENTS.CODE_ARTICLE = {Param_code_produit} AND'
        if kwargs['Param_code_produit'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_ls_operateurs(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_OPERATEUR.ACTIF = {Param_actif}'

        if kwargs['Param_actif'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''

        return query.format(**kwargs).format(**kwargs)

    def Req_ls_operateurs_fonction(self, args):
        query = '''
            SELECT 
                T_OPERATEUR.CODE_OPERATEUR AS CODE_OPERATEUR,	
                T_OPERATEUR.Matricule AS Matricule,	
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND	T_OPERATEUR.FONCTION IN ({Param_fonction})'

        if kwargs['Param_fonction'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_ls_operations_caisse(self, args):
        ...
        try:
            kwargs = {
                ...
                'Param_type_operation': '\',\''.join(args[3].split(',')),
        ...
        kwargs['Param_dt2'] = self.validateDate(kwargs['Param_dt2'])
        kwargs['Param_date_validation'] = self.validateDate(kwargs['Param_date_validation'])
        kwargs['Param_diff_validation'] = self.validateDate(kwargs['Param_diff_validation'])

        for key in ('Param_dt1', 'Param_dt2', 'Param_type_operation'):
            if kwargs[key] in self.null_values:
                return ValueError
        ...
        kwargs['OPTIONAL_ARG_2'] = 'AND T_OPERATIONS_CAISSE.DATE_VALIDATION <> (\'{Param_diff_validation}\')'
        if kwargs['Param_diff_validation'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''
    ...
    def Req_ls_operations_non_justifies(self, args):
        ...
        try:
            kwargs = {
                'Param_date_operation': args[0],
                'Param_type_operation': '\',\''.join(args[1].split(',')),
                'CODE_AGCE': args[len(args) - 1]
            }
        except IndexError as e:
            return e
    ...
    def Req_ls_prelev_journee(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_PRELEVEMENT_SUSP_COND.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_ls_prelevement(self, args):
        query = '''
            ...
            FROM 
                T_PRELEVEMENT_SUSP_COND
            WHERE
                T_PRELEVEMENT_SUSP_COND.CODE_AGCE = {CODE_AGCE}
            ORDER BY 
                ID_PRELEV DESC
        '''
    ...
    def Req_ls_prelevements_periode(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_DT_PRELEVEMENT_COND.CODE_OPERATEUR = {Param_code_operateur}
    ...
    def Req_ls_prelevements_periode_caisserie(self, args):
        query = '''
            SELECT 
                T_DT_PRELEVEMENT_COND.CODE_OPERATEUR AS CODE_OPERATEUR,	
                T_PRELEVEMENT_SUSP_COND.DATE_VALIDATION AS DATE_VALIDATION,	
    ...
    def Req_ls_preparation(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_ARTICLES.CODE_ARTICLE = T_ARTICLES_AGENCE.CODE_ARTICLE
                AND T_ARTICLES_AGENCE.CODE_AGCE = {CODE_AGCE}
                AND T_PREPARATION_CHARGEMENTS.CODE_AGCE = {CODE_AGCE}
    ...
    def Req_ls_produit_commandes(self, args):
        query = '''
            SELECT 
                T_PRODUITS_COMMANDES.ID_COMMANDE AS ID_COMMANDE,	
                T_PRODUITS_COMMANDES.CODE_ARTICLE AS CODE_ARTICLE,	
            ...
            WHERE 
                ...
                AND T_PRODUITS_COMMANDES.ID_COMMANDE = {Param_id_commande}
    ...
    def Req_ls_produits_chargees(self, args):
        query = '''
            ...
            WHERE 
                T_PRODUITS_CHARGEE.CODE_CHARGEMENT = T_CHARGEMENT.CODE_CHARGEMENT
                AND T_CHARGEMENT.CODE_CHARGEMENT = {Param_code_chagement}
                {OPTIONAL_ARG_1}
                AND T_CHARGEMENT.CODE_AGCE = {CODE_AGCE}
        '''
        ...
        if kwargs['Param_code_chagement'] in self.null_values:
            return ValueError
    ...
    def Req_ls_produits_livraison(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_PRODUITS_LIVREES.NUM_LIVRAISON = {Param_num_livraison}
                AND T_ARTICLES_AGENCE.CODE_AGCE = {CODE_AGCE}
            ORDER BY 
                RANG ASC
        '''
    ...
    def Req_ls_reclamations(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_RECLAMATIONS_QUALITE.DATE_RECLAMATION BETWEEN '{Param_date1}' AND '{Param_date2}'
                {OPTIONAL_ARG_1}
                AND T_RECLAMATIONS_QUALITE.CODE_AGCE = {CODE_AGCE}
        '''
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_RECLAMATIONS_QUALITE.DATE_HEURE BETWEEN \'{Param_dts1}\' AND \'{Param_dts2}\''

        for key in ('Param_dts1', 'Param_dts2'):
            if kwargs[key] in self.null_values:
                kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_ls_reconaissances(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_RECONAISSANCES.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_ls_reglement(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_DECOMPTE.REGLEMENT = {Param_reglement}
                {OPTIONAL_ARG_1}
                {OPTIONAL_ARG_2}
                AND	T_DECOMPTE.MODE_PAIEMENT <> 'R'
                AND T_DECOMPTE.CODE_AGCE = {CODE_AGCE}
        ...
        if kwargs['Param_reglement'] in self.null_values:
            return ValueError
        
        kwargs['OPTIONAL_ARG_1'] = 'AND T_DECOMPTE.DATE_VALIDATION BETWEEN \'{Param_dt1}\' AND \'{Param_dt2}\''
        if kwargs['Param_dt1'] in self.null_values or kwargs['Param_dt2'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_ls_reglements(self, args):
        ...
        if kwargs['Param_dt_reglement'] in self.null_values:
            return ValueError
    ...
    def Req_ls_reglements_cond(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_REGELEMENT_COND.CODE_OPERTAEUR = {Param_code_operateur}'
        if kwargs['Param_code_operateur'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_ls_remise_clients(self, args):
        query = '''
            SELECT 
                T_REMISE_CLIENT_TRAD.Date_Debut AS Date_Debut,	
                T_REMISE_CLIENT_TRAD.Date_Fin AS Date_Fin,	
                T_REMISE_CLIENT_TRAD.CODE_CLIENT AS CODE_CLIENT,	
                T_REMISE_CLIENT_TRAD.DATE_REMISE AS DATE_REMISE,	
                T_REMISE_CLIENT_TRAD.MT_REMISE AS MT_REMISE,	
                T_REMISE_CLIENT_TRAD.MT_REPARTI AS MT_REPARTI,	
                T_REMISE_CLIENT_TRAD.CODE_OPERATEUR AS CODE_OPERATEUR,	
                T_REMISE_CLIENT_TRAD.MT_CA AS MT_CA,	
                T_REMISE_CLIENT_TRAD.VALID AS VALID,	
                T_SOUS_SECTEUR.code_secteur AS code_secteur,	
                T_CLIENTS.NOM_CLIENT AS NOM_CLIENT,	
                T_SECTEUR.NOM_SECTEUR AS NOM_SECTEUR,	
                T_REMISE_CLIENT_TRAD.TX_DERIVES AS TX_DERIVES,	
                T_REMISE_CLIENT_TRAD.CA_MOY AS CA_MOY,	
                T_ZONE.CODE_SUPERVISEUR AS CODE_SUPERVISEUR,	
                T_ZONE.RESP_VENTE AS RESP_VENTE,	
                T_OPERATEUR.NOM_OPERATEUR AS NOM_SUPERVISEUR,	
                T_RESP_VENTE.NOM_OPERATEUR AS NOM_ANIMATEUR,	
                T_CLIENTS.TYPE_PRESENTOIRE AS TYPE_PRESENTOIRE,	
                T_REMISE_CLIENT_TRAD.MT_REMISE_LAIT AS MT_REMISE_LAIT,	
                T_REMISE_CLIENT_TRAD.TX_LAIT AS TX_LAIT,	
                T_REMISE_CLIENT_TRAD.STATUT AS STATUT
            FROM 
                T_REMISE_CLIENT_TRAD,	
                T_CLIENTS,	
            ...
            WHERE 
                ...
                AND T_REMISE_CLIENT_TRAD.CODE_CLIENT = T_CLIENTS.CODE_CLIENT
                AND T_REMISE_CLIENT_TRAD.Date_Debut BETWEEN '{Param_dt1}' AND '{Param_dt2}'
                {OPTIONAL_ARG_1}
                {OPTIONAL_ARG_2}
                {OPTIONAL_ARG_3}
                AND	T_REMISE_CLIENT_TRAD.STATUT >= 0
                AND T_ZONE.CODE_AGCE = {CODE_AGCE}
                AND T_CLIENTS.CODE_AGCE = {CODE_AGCE}
            ORDER BY 
                NOM_ANIMATEUR ASC,	
        ...
        try:
            kwargs = {
                ...
                'Param_dt2': args[1],
                'Param_code_client': args[2],
                'Param_code_secteur': args[3],
                'Param_code_superviseur': args[4],
                'CODE_AGCE': args[len(args) - 1]
            }
        except IndexError as e:
            return e
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND	T_REMISE_CLIENT_TRAD.CODE_CLIENT = {Param_code_client}'
        kwargs['OPTIONAL_ARG_1'] = '' if kwargs['Param_code_client'] in self.null_values else kwargs['OPTIONAL_ARG_1']
        kwargs['OPTIONAL_ARG_2'] = 'AND	T_SOUS_SECTEUR.code_secteur = {Param_code_secteur}'
        kwargs['OPTIONAL_ARG_2'] = '' if kwargs['Param_code_secteur'] in self.null_values else kwargs['OPTIONAL_ARG_2']
        kwargs['OPTIONAL_ARG_3'] = 'AND	T_ZONE.CODE_SUPERVISEUR = {Param_code_superviseur}'
        kwargs['OPTIONAL_ARG_3'] = '' if kwargs['Param_code_superviseur'] in self.null_values else kwargs['OPTIONAL_ARG_3']
    ...
    def Req_ls_remises_clients(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND	T_SECTEUR.code_secteur = T_SOUS_SECTEUR.code_secteur
                AND	T_SOUS_SECTEUR.CODE_SOUS_SECTEUR = T_CLIENTS.SOUS_SECTEUR
                AND	T_CLIENTS.CODE_CLIENT = T_REMISE_CLIENT_TRAD.CODE_CLIENT
                AND T_REMISE_CLIENT_TRAD.Date_Debut = '{Param_date_debut}'
                {OPTIONAL_ARG_1}
                {OPTIONAL_ARG_2}
                {OPTIONAL_ARG_3}
                AND T_ZONE.CODE_AGCE = {CODE_AGCE}
        ...
        kwargs['OPTIONAL_ARG_1'] = '' if kwargs['Param_code_secteur'] in self.null_values else kwargs['OPTIONAL_ARG_1']
        kwargs['OPTIONAL_ARG_2'] = 'AND	T_ZONE.CODE_SUPERVISEUR = {Param_code_superviseur}'
        kwargs['OPTIONAL_ARG_2'] = '' if kwargs['Param_code_superviseur'] in self.null_values else kwargs['OPTIONAL_ARG_2']
        kwargs['OPTIONAL_ARG_3'] = 'AND	T_ZONE.RESP_VENTE = {Param_code_resp_vente}'
        kwargs['OPTIONAL_ARG_3'] = '' if kwargs['Param_code_resp_vente'] in self.null_values else kwargs['OPTIONAL_ARG_3']
    ...
    def Req_ls_remises_par_secteur(self, args):
        query = '''
            ...
            FROM 
                ...
                T_CLIENTS
            WHERE 
                T_CLIENTS.CODE_CLIENT = T_REMISE_CLIENT_TRAD.CODE_CLIENT
                AND T_ZONE.CODE_ZONE = T_BLOC.CODE_ZONE
                AND T_OPERATEUR.CODE_OPERATEUR = T_ZONE.CODE_SUPERVISEUR
                AND T_BLOC.CODE_BLOC = T_SECTEUR.CODE_BLOC
                AND T_SECTEUR.code_secteur = T_SOUS_SECTEUR.code_secteur
    ...
    def Req_ls_secteur(self, args):
        query = '''
            ...
            FROM 
                ...
                T_OPERATEUR
            WHERE 
                T_OPERATEUR.CODE_OPERATEUR = T_ZONE.CODE_SUPERVISEUR
                AND T_BLOC.CODE_BLOC = T_SECTEUR.CODE_BLOC
                AND T_ZONE.CODE_ZONE = T_BLOC.CODE_ZONE
                AND T_SECTEUR.ACTIF = 1
                {OPTIONAL_ARG_1}
                AND T_ZONE.CODE_AGCE = {CODE_AGCE}
            ORDER BY 
                RANG ASC
    ...
    def Req_ls_secteur_sans_commande(self, args):
        ...
        if kwargs['Param_cds'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''

        return query.format(**kwargs).format(**kwargs)

    def Req_ls_ss_secteur(self, args):
        query = '''
            SELECT 
    ...
    def Req_ls_ss_tournee(self, args):
        query = '''
            SELECT 
                T_SOUS_SECTEUR.CODE_SOUS_SECTEUR AS CODE_SOUS_SECTEUR,	
                T_SOUS_SECTEUR.NOM_SOUS_SECTEUR AS NOM_SOUS_SECTEUR,	
    ...
    def Req_ls_superviseurs(self, args):
        query = '''
            SELECT 
                T_OPERATEUR.CODE_OPERATEUR AS CODE_OPERATEUR,	
                T_OPERATEUR.NOM_OPERATEUR AS NOM_OPERATEUR,	
    ...
    def Req_ls_superviseurs_resp_vente(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND	T_ZONE.RESP_VENTE = {param_resp_vente}'
    ...
    def Req_ls_tache_operateur(self, args):
        query = '''
            SELECT 
                T_OPERTEURS_TACHES.CODE_OPERATEUR AS CODE_OPERATEUR,	
                T_OPERTEURS_TACHES.ID_TACHE AS ID_TACHE,	
    ...
    def Req_ls_vehicule(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND	T_VEHICULE.Id_Type_Vehicule = {ParamId_Type_Vehicule}'
        if kwargs['ParamId_Type_Vehicule'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_ls_versement_caisse(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_OPERATIONS_CAISSE.DATE_VALIDATION = \'{Param_date_versement}\''

        if kwargs['Param_date_versement'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_magasin_categorie(self, args):
        query = '''
            SELECT 
            ...
            WHERE 
                ...
                AND T_TYPE_PRODUIT_MAG.PRODUIT = '{Param_categorie}'
                AND T_MAGASINS.CODE_AGCE = {CODE_AGCE}       
        '''
    ...
    def Req_magasin_cond(self, args):
        query = '''
            SELECT 
                T_MAGASIN_COND.CODE_MAGASIN AS CODE_MAGASIN,	
                T_MAGASIN_COND.CODE_CP AS CODE_CP,	
    ...
    def Req_maj_position_cond(self, args):
        query = '''
            ...
            FROM
                T_MAGASIN_COND mc
                INNER JOIN T_MAGASINS m ON m.CODE_MAGASIN = mc.CODE_MAGASIN
            SET
                mc.QTE_STOCK = {Param_qte_stock}
            WHERE
                mc.CODE_CP = {Param_code_cp}
                AND m.CODE_AGCE = {CODE_AGCE}
                {OPTIONAL_ARG_1}
        '''
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND mc.CODE_MAGASIN = {Param_code_magasin}'
    ...
    def Req_max_autorisation(self, args):
        query = '''
            ...
            FROM 
                ...
                T_OPERATEUR
            WHERE
                T_OPERATEUR.CODE_OPERATEUR = T_AUTORISATIONS_SOLDE.CODE_OPERATEUR
                AND T_OPERATEUR.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_max_commandes_usine(self, args):
        query = '''
            ...
            FROM 
                T_COMMANDES
            WHERE
                T_COMMANDES.CODE_AGENCE = {CODE_AGCE}
        '''
    ...
    def Req_max_cond_livrees(self, args):
        query = '''
            ...
            FROM 
                T_COND_LIVRAISON
            WHERE
                T_COND_LIVRAISON.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_max_convoyage(self, args):
        query = '''
            ...
            FROM 
                T_CONVOYAGE
            WHERE
                T_CONVOYAGE.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_max_decompte(self, args):
        query = '''
            ...
            FROM 
                T_DECOMPTE
            WHERE
                T_DECOMPTE.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_max_envoi(self, args):
        query = '''
            ...
            FROM 
                T_COURRIER_AGENCE
            WHERE
                T_COURRIER_AGENCE.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_max_livraison(self, args):
        query = '''
            SELECT 
            ...
            FROM 
                T_LIVRAISON
            WHERE
                T_LIVRAISON.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_max_mvt_caisse(self, args):
        query = '''
            ...
            FROM 
                ...
                T_CAISSE
            WHERE
                T_MOUVEMENTS_CAISSE.CODE_CAISSE = T_CAISSE.CODE_CAISSE
                AND T_CAISSE.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_max_operations_caisse(self, args):
        query = '''
            ...
            FROM 
                T_OPERATIONS_CAISSE
            WHERE
                T_OPERATIONS_CAISSE.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_max_reconaissance(self, args):
        query = '''
            ...
            FROM 
                T_RECONAISSANCES
            WHERE
                T_RECONAISSANCES.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_max_reglement(self, args):
        query = '''
            ...
            FROM 
                T_REGELEMENT_COND
            WHERE
                T_REGELEMENT_COND.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_modele_taches(self, args):
        ...
        if kwargs['Param_id_modele'] in self.null_values:
            return ValueError
    ...
    def Req_montant_a_verser(self, args):
        query = '''
            SELECT 
                T_CHARGEMENT.DATE_CHARGEMENT AS DATE_CHARGEMENT,	
                T_CHARGEMENT.code_vendeur AS CODE_OPERATEUR,	
                T_CHARGEMENT.VALID AS VALID,	
    ...
    def Req_mouvements_caisserie(self, args):
        query = '''
            SELECT 
                T_CHARGEMENT.DATE_CHARGEMENT AS DATE_CHARGEMENT,	
                T_COND_CHARGEE.code_secteur AS code_secteur,	
    ...
    def Req_moy_vente_gms(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_PRODUITS.CODE_PRODUIT = T_ARTICLES.CODE_PRODUIT
                AND T_PRODUITS_LIVREES.DATE_VALIDATION BETWEEN '{Param_dt1}' AND '{Param_dt2}'
    ...
    def Req_moyenne_vente_produit_secteur(self, args):
        ...
        if kwargs['Param_code_famille'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''
        
        return query.format(**kwargs).format(**kwargs)

    def Req_mt_a_verser_secteur(self, args):
        query = '''
            SELECT 
                T_CHARGEMENT.DATE_CHARGEMENT AS DATE_CHARGEMENT,	
                T_CHARGEMENT.code_secteur AS code_secteur,	
    ...
    def Req_mt_verse_operateur(self, args):
        query = '''
            SELECT 
                T_DECOMPTE.CODE_OPERATEUR AS CODE_OPERATEUR,	
                T_DECOMPTE.MODE_PAIEMENT AS MODE_PAIEMENT,	
    ...
    """def Req_mvente(self, args):
        ...
        if kwargs['Param_date_vente'] in self.null_values:
            return ValueError
    ...
    def Req_objectif_agence(self, args):
        ...
        if kwargs['Param_dt_objectif'] in self.null_values:
            return ValueError
    ...
    def Req_objectif_clients(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_OBJECTIF_CLIENTS.DATE_OBJECTIF = '{Param_date_objectif}'
    ...
    def Req_objectif_mois(self, args):
        query = '''
            ...
            FROM 
                T_OBJECTIF_VENTE
                INNER JOIN T_SECTEUR
                  ON T_SECTEUR.CODE_SECTEUR = T_OBJECTIF_VENTE.CODE_SECTEUR
                INNER JOIN T_BLOC ON T_BLOC.CODE_BLOC = T_SECTEUR.CODE_BLOC
                INNER JOIN T_ZONE ON T_ZONE.CODE_ZONE = T_BLOC.CODE_ZONE
                    AND T_ZONE.CODE_AGCE = {CODE_AGCE}
            WHERE 
        ...
        if kwargs['Param_date_objectif'] in self.null_values:
            return ValueError
    ...
    def Req_objectif_secteurs(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_OBJECTIF_SECTEURS.DATE_OBJECTIF = '{Param_date_objectif}'
                {OPTIONAL_ARG_1}
        '''
    ...
    def Req_objectifs_clients(self, args):
        ...
        kwargs['OPTIONAL_ARG_3'] = '' if kwargs['Param_code_secteur'] in self.null_values else kwargs['OPTIONAL_ARG_3']

        return query.format(**kwargs).format(**kwargs)

    def Req_position_cond(self, args):
        query = '''
            SELECT 
                T_MAGASIN_COND.CODE_MAGASIN AS CODE_MAGASIN,	
                T_MAGASIN_COND.CODE_CP AS CODE_CP,	
    ...
    def Req_position_stock(self, args):
        ...
        if kwargs['Param_type_produit'] in self.null_values:
            return ValueError
        
        kwargs['OPTIONAL_ARG_1'] = 'AND T_ARTICLES_MAGASINS.CODE_ARTICLE = {Param_code_article}'
        kwargs['OPTIONAL_ARG_2'] = 'AND	T_ARTICLES_MAGASINS.MAGASIN = {Param_code_magasin}'
    ...
    def Req_prix_article_periode(self, args):
        query = '''
            SELECT 
                T_PRIX.Date_Debut AS Date_Debut,	
                T_PRIX.Date_Fin AS Date_Fin,	
    ...
    def Req_produits_famille(self, args):
        query = '''
            SELECT 
                T_PRODUITS.CODE_PRODUIT AS CODE_PRODUIT,	
        ...
        kwargs['OPTIONAL_ARG_1'] = 'WHERE T_PRODUITS.CODE_FAMILLE = {Param_code_famille}'
    ...
    def Req_produits_mappage(self, args):
        query = '''
            ...
            FROM 
                ...
                T_ARTICLES_AGENCE
            WHERE 
                T_ARTICLES_AGENCE.CODE_ARTICLE = T_ARTICLES.CODE_ARTICLE
                AND T_ARTICLES_AGENCE.ACTIF = 1
                AND T_ARTICLES_AGENCE.CODE_AGCE = {CODE_AGCE}
            GROUP BY 
                T_ARTICLES_AGENCE.ACTIF,	
    ...
    def Req_promotions_dt(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_PROMOTIONS.Date_Debut <= '{Param_dt}'
                AND	T_PROMOTIONS.Date_Fin >= '{Param_dt}'
                AND	
                    (
                        T_CIBLE_PROMOTION.code_secteur = {CODE_0}
    ...
    def Req_qte_commande(self, args):
        ...
        for kwarg in ('Param_code_secteur', 'Param_date_livraison'):
            if kwargs[kwarg] in self.null_values:
                return ValueError
    ...
    def Req_realisation_globale(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = '' if kwargs['param_code_famille'] in self.null_values else kwargs['OPTIONAL_ARG_1']
        kwargs['OPTIONAL_ARG_2'] = 'AND	T_PRODUITS.CODE_PRODUIT = {param_code_produit}'
        kwargs['OPTIONAL_ARG_2'] = '' if kwargs['param_code_produit'] in self.null_values else kwargs['OPTIONAL_ARG_2']
        kwargs['OPTIONAL_ARG_3'] = 'AND	T_PRODUITS.CAT_PRODUIT = {param_cat_produit}'
        kwargs['OPTIONAL_ARG_3'] = '' if kwargs['param_cat_produit'] in self.null_values else kwargs['OPTIONAL_ARG_3']
        kwargs['OPTIONAL_ARG_4'] = 'AND	T_ZONE.CODE_SUPERVISEUR = {param_code_superviseur}'
        kwargs['OPTIONAL_ARG_4'] = '' if kwargs['param_code_superviseur'] in self.null_values else kwargs['OPTIONAL_ARG_4']
        kwargs['OPTIONAL_ARG_5'] = 'AND	T_ZONE.RESP_VENTE = {param_code_resp_vente}'
        kwargs['OPTIONAL_ARG_5'] = '' if kwargs['param_code_resp_vente'] in self.null_values else kwargs['OPTIONAL_ARG_5']
    ...
    def Req_recap_factures_produits(self, args):
        ...
        kwargs['OPTIONAL_ARG_4'] = '' if kwargs['Param_code_secteur'] in self.null_values else kwargs['OPTIONAL_ARG_4']
        kwargs['OPTIONAL_ARG_5'] = 'AND	T_CLIENTS.CLASSE = {param_eg_classe}'
        kwargs['OPTIONAL_ARG_5'] = '' if kwargs['param_eg_classe'] in self.null_values else kwargs['OPTIONAL_ARG_5']
        kwargs['OPTIONAL_ARG_6'] = 'AND	T_CLIENTS.CLASSE <> {param_diff_classe}'
        kwargs['OPTIONAL_ARG_6'] = '' if kwargs['param_diff_classe'] in self.null_values else kwargs['OPTIONAL_ARG_6']
        kwargs['OPTIONAL_ARG_7'] = 'AND	T_PRODUITS.CODE_FAMILLE = {param_code_famille}'
        kwargs['OPTIONAL_ARG_7'] = '' if kwargs['param_code_famille'] in self.null_values else kwargs['OPTIONAL_ARG_7']
    ...
    """def Req_recensement_clts_nt(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = '' if kwargs['Param_egale'] in self.null_values else kwargs['OPTIONAL_ARG_1']
        kwargs['OPTIONAL_ARG_2'] = 'T_RECENSEMENT.CODE_CLIENT <> {Param_diff} AND'
        kwargs['OPTIONAL_ARG_2'] = '' if kwargs['Param_diff'] in self.null_values else kwargs['OPTIONAL_ARG_2']
        kwargs['OPTIONAL_ARG_3'] = 'T_RECENSEMENT.VALID = {Param_valid}'
        kwargs['OPTIONAL_ARG_3'] = '' if kwargs['Param_valid'] in self.null_values else kwargs['OPTIONAL_ARG_3']

        return query.format(**kwargs).format(**kwargs)"""

    def Req_recherche_client_code_interne(self, args):
        query = '''
            SELECT 
                T_CLIENTS.CODE_CLIENT AS CODE_CLIENT,	
                T_CLIENTS.NOM_CLIENT AS NOM_CLIENT,	
        ...
        if kwargs['Param_code_in'] in self.null_values:
            return ValueError

        return query.format(**kwargs)

    def Req_recherche_operateur(self, args):
        query = '''
            SELECT 
                T_OPERATEUR.CODE_OPERATEUR AS CODE_OPERATEUR,	
                T_OPERATEUR.Matricule AS Matricule,	
        ...
        if kwargs['Param_nom_operateur'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
        
        if kwargs['Param_code_fonction'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''
    ...
    def Req_recherche_par_matricule(self, args):
        ...
        if kwargs['Param_matricule'] in self.null_values:
            return ValueError
    ...
    def Req_recherche_prevision(self, args):
        query = '''
            SELECT 
                T_PREVISION.Date_Debut AS Date_Debut,	
                T_PREVISION.Date_Fin AS Date_Fin,	
    ...
    def Req_recherche_secteur(self, args):
        ...
        if kwargs['Param_nom_secteur'] in self.null_values:
            return ValueError
    ...
    def Req_reconaissances_operateur(self, args):
        query = '''
            ...
            FROM 
                T_RECONAISSANCES
            WHERE 
                T_RECONAISSANCES.DATE_RECONAISS = '{Param_date_recon}'
                AND T_RECONAISSANCES.CODE_AGCE = {CODE_AGCE}
        '''
        ...
        kwargs['Param_date_recon'] = self.validateDate(kwargs['Param_date_recon'])
    ...
    def Req_releve_client_cac(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_OPERATEUR.CODE_OPERATEUR = T_LIVRAISON.code_vendeur
                AND T_GROUP_CLIENT.ID_GROUP = T_CLIENTS.GROUP_CLIENT
                AND T_LIVRAISON.TYPE_MVT IN ('L', 'R') 
                {OPTIONAL_ARG_1}
                AND	T_LIVRAISON.STATUT <> 'A'
                {OPTIONAL_ARG_2}
        ...
        kwargs['OPTIONAL_ARG_3'] = 'AND T_LIVRAISON.DATE_VALIDATION BETWEEN \'{Param_dt1}\' AND \'{Param_dt2}\''

        if kwargs['Param_code_client'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''

        if None in (kwargs['Param_dtl1'], kwargs['Param_dtl2']):
            kwargs['OPTIONAL_ARG_2'] = ''

        if None in (kwargs['Param_dt1'], kwargs['Param_dt2']):
            kwargs['OPTIONAL_ARG_3'] = ''
    ...
    def Req_releve_client_details_produits(self, args):
        query = '''
            SELECT 
                T_FACTURE.NUM_FACTURE AS NUM_FACTURE,	
                T_FACTURE.CODE_CLIENT AS CODE_CLIENT,	
    ...
    def Req_releve_client_global_produit(self, args):
        query = '''
            SELECT 
                T_FACTURE.CODE_CLIENT AS CODE_CLIENT,	
                MIN(T_ARTICLES.RANG) AS le_minimum_RANG,	
    ...
    def Req_releve_client_globale(self, args):
        query = '''
            SELECT 
                T_FACTURE.NUM_FACTURE AS NUM_FACTURE,	
                T_FACTURE.CODE_CLIENT AS CODE_CLIENT,	
    ...
    def Req_releve_client_tva(self, args):
        query = '''
            SELECT 
                T_FACTURE.CODE_CLIENT AS CODE_CLIENT,	
                T_CLIENTS.NOM_CLIENT AS NOM_CLIENT,	
    ...
    def Req_releve_dons(self, args):
        ...
        if kwargs['param_code_sect'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_relve_cac(self, args):
        query = '''
            SELECT 
                ...
                T_GROUP_CLIENT.NOM_GROUP AS ENSEIGNE,	
                T_CLIENTS.CODE_CLIENT AS CODE_CLIENT,	
                T_CLIENTS.NOM_CLIENT AS NOM_CLIENT,	
                T_OPERATEUR.NOM_OPERATEUR AS NOM_VENDEUR,	
                T_LIVRAISON.NUM_LIVRAISON AS NUM_LIVRAISON,	
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_LIVRAISON.DATE_VALIDATION BETWEEN \'{Param_dt1}\' AND \'{Param_dt2}\''
        kwargs['OPTIONAL_ARG_2'] = 'AND T_LIVRAISON.DATE_LIVRAISON BETWEEN \'{Param_dtj1}\' AND \'{Param_dtj2}\''

        if kwargs['Param_dt1'] in self.null_values or kwargs['Param_dt2'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
        
        if kwargs['Param_dtj1'] in self.null_values or kwargs['Param_dtj2'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''

        return query.format(**kwargs).format(**kwargs)

    def Req_remarque_journee(self, args):
        query = '''
            SELECT 
                T_JOURNEE.DATE_JOURNEE AS DATE_JOURNEE,	
    ...
    def Req_remise_client(self, args):
        query = '''
            ...
            WHERE 
                T_REMISE_CLIENT_TRAD.Date_Debut = '{Param_DATE_DEBUT}'
                AND	T_REMISE_CLIENT_TRAD.CODE_CLIENT = {Param_CODE_CLIENT}
        '''
    ...
    def Req_remise_clt(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_DT_REMISE_CLASSE.CODE_CLASSE = {Param_code_classe}
                AND	T_FACTURE.CODE_CLIENT = {Param_code_client}
    ...
    def Req_repartition(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND	T_REPARTITION.CODE_AGENCE = {CODE_AGCE}
        '''
        ...
        kwargs['Param_date_repartition'] = self.validateDate(
            kwargs['Param_date_repartition'])

        if kwargs['Param_date_repartition'] in self.null_values:
            return ValueError
    ...
    def Req_sit_caisserie(self, args):
        query = '''
            ...
            WHERE 
                T_OPERATIONS.CODE_OPERATION = T_MOUVEMENTS_CAISSERIE.ORIGINE
                AND T_OPERATIONS.TYPE_OPERATION IN ('R', 'T') 
    ...
    def Req_solde_initial_caisse(self, args):
        query = '''
            ...
            WHERE 
                ...
                {OPTIONAL_ARG_1}
                AND T_SOLDE_INITIAL_CAISSE.CODE_CAISSE = T_CAISSE.CODE_CAISSE
	            AND T_CAISSE.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_solde_initial_client(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_SOLDE_INITIAL_CLIENT.DATE_JOURNEE = '{Param_date_journee}'
                {OPTIONAL_ARG_1}
                AND T_SOLDE_INITIAL_CLIENT.CODE_AGCE = {CODE_AGCE}
        '''
        ...
        if kwargs['Param_client'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_solde_operateur(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_SOLDE_INITIAL.DATE_JOURNEE = '{Param_date_journee}'
                {OPTIONAL_ARG_1}
                AND T_SOLDE_INITIAL.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_ss_tournee(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_TOURNEES_SS.CODE_TOURNEE = {Param_code_tournee}
        '''
    ...
    def Req_stock_article_magasin(self, args):
        query = '''
            SELECT 
                T_ARTICLES_MAGASINS.CODE_ARTICLE AS CODE_ARTICLE,	
                T_ARTICLES_MAGASINS.MAGASIN AS MAGASIN,	
    ...
    def Req_stock_inital_cond(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_STOCK_INITI_COND.CODE_AGCE = {CODE_AGCE}
            GROUP BY 
    ...
    def Req_stock_initial(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_STOCK_INIT.CODE_AGCE = {CODE_AGCE}
            GROUP BY 
    ...
    def Req_stock_produit_magasin(self, args):
        query = '''
            SELECT 
                T_ARTICLES_MAGASINS.CODE_ARTICLE AS CODE_ARTICLE,	
                SUM(T_ARTICLES_MAGASINS.QTE_STOCK) AS la_somme_QTE_STOCK
        ...
        if kwargs['Paramcode_magasin'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_sup_chargement_secteur(self, args):
        query = '''
            DELETE pc FROM 
                T_CHARGEMENT c
                INNER JOIN T_PRODUITS_CHARGEE pc ON c.CODE_CHARGEMENT = pc.CODE_CHARGEMENT
            WHERE 
                c.DATE_CHARGEMENT = '{Param_date_chargement}'
                AND	c.code_secteur = {Param_code_secteur}
                AND c.CODE_AGCE = {CODE_AGCE}

            DELETE cc FROM 
                T_CHARGEMENT c
                INNER JOIN T_COND_CHARGEE cc ON c.CODE_CHARGEMENT = cc.CODE_CHARGEMENT
            WHERE 
                c.DATE_CHARGEMENT = '{Param_date_chargement}'
                AND	c.code_secteur = {Param_code_secteur}
                AND c.CODE_AGCE = {CODE_AGCE}
    ...
    """def Req_supp_bl_mission(self, args):
        ...
        if kwargs['Param_id_det'] in self.null_values:
            return ValueError
    ...
    def Req_supp_cond_livree(self, args):
        ...
        if kwargs['Param_nbl'] in self.null_values:
            return ValueError

        return query.format(**kwargs)

    def Req_supp_dt_reclamation(self, args):
        query = '''
            DELETE FROM 
                T_DT_RECLAMATION
            WHERE 
    ...
    def Req_supp_hist_clients(self, args):
        query = '''
            DELETE FROM 
                T_MOY_VENTE_CLIENTS
            WHERE 
    ...
    def Req_supp_hist_secteur(self, args):
        query = '''
            DELETE FROM 
                T_MOY_VENTE_ARTICLE
            WHERE 
                T_MOY_VENTE_ARTICLE.DATE_VENTE = '{Param_date_journee}'
                AND T_MOY_VENTE_ARTICLE.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_supp_itineraire(self, args):
        query = '''
            DELETE FROM 
                T_ITINERAIRES
            WHERE 
    ...
    def Req_supp_justification_caisserie(self, args):
        query = '''
            DELETE FROM 
                T_AUTORISATION_SOLDE_CAISSERIE
            WHERE 
        ...
        if kwargs['Param_id_justification'] in self.null_values:
            return ValueError
    ...
    def Req_supp_mouvement_operation(self, args):
        query = '''
            DELETE FROM 
                T_MOUVEMENTS
            WHERE 
        ...
        if kwargs['Param_code_operation'] in self.null_values:
            return ValueError
    ...
    def Req_supp_moy_vente_clients(self, args):
        query = '''
            DELETE FROM 
                T_MOY_VENTE_CLIENTS
            WHERE 
    ...
    """def Req_supp_mvente(self, args):
        ...
        if kwargs['Param_date_mvente'] in self.null_values:
            return ValueError
    ...
    def Req_supp_obj_secteurs(self, args):
        query = '''
            DELETE FROM 
                T_OBJECTIF_SECTEURS o
                INNER JOIN T_SECTEUR s ON s.CODE_SECTEUR = o.CODE_SECTEUR
                INNER JOIN T_BLOC b ON b.code_bloc = s.code_bloc
                INNER JOIN T_ZONE z ON z.CODE_ZONE = b.CODE_ZONE
            WHERE 
                T_OBJECTIF_SECTEURS.DATE_OBJECTIF = '{Param_date}'
                AND z.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_supp_objectif(self, args):
        query = '''
            DELETE FROM 
                T_OBJECTIF_VENTE o
                INNER JOIN T_SECTEUR s ON s.CODE_SECTEUR = o.CODE_SECTEUR
    ...
    def Req_supp_objectif_clients(self, args):
        query = '''
            DELETE FROM 
                T_OBJECTIF_CLIENTS
            WHERE 
    ...
    def Req_supp_objectif_rendus(self, args):
        query = '''
            DELETE FROM 
                T_OBJECTIF_RENDUS
            WHERE 
    ...
    def Req_supp_produit_comm(self, args):
        query = '''
            DELETE FROM 
                T_PRODUITS_COMMANDES
            WHERE 
    ...
    def Req_supp_produits_chargees(self, args):
        query = '''
            DELETE FROM 
                T_PRODUITS_CHARGEE pc
                INNER JOIN T_CHARGEMENT c ON pc.CODE_CHARGEMENT = c.CODE_CHARGEMENT
            WHERE 
                c.CODE_SECTEUR = {Param_code_secteur}
                AND	c.DATE_CHARGEMENT = '{Param_date_chargement}'
                AND c.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_supp_solde_caisse(self, args):
        query = '''
            DELETE FROM 
                T_SOLDE_INITIAL_CAISSE s
                INNER JOIN T_CAISSE c ON c.CODE_CAISSE = s.CODE_CAISSE
            WHERE 
                s.DATE_JOURNEE = '{Param_date_journee}'
                AND c.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_supp_solde_init_caisse(self, args):
        return self.Req_supp_solde_caisse(args)

    def Req_supp_solde_init_client(self, args):
        query = '''
            DELETE FROM 
                T_SOLDE_INITIAL_CLIENT
            WHERE 
        ...
        for kwarg in ('Param_date_journee', 'CODE_AGCE'):
            if kwargs[kwarg] in self.null_values:
                return ValueError
    ...
    def Req_supp_solde_initial_operateur(self, args):
        query = '''
            DELETE FROM 
                T_SOLDE_INITIAL
            WHERE 
    ...
    def Req_supp_statistiques_clients(self, args):
        query = '''
            DELETE FROM 
                STATISTIQUES.dbo.T_STATISTIQUES
            WHERE 
    ...
    def Req_supp_statistiques_stock(self, args):
        query = '''
            DELETE FROM 
    ...
    def Req_supp_stock_init_cond(self, args):
        query = '''
            DELETE FROM 
                T_STOCK_INITI_COND
            WHERE 
    ...
    def Req_supp_stock_initial(self, args):
        query = '''
            DELETE FROM 
                T_STOCK_INIT
            WHERE 
    ...
    def req_susp_cond_chargement_journee(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_COND_CHARGEE.DATE_CHARGEMENT = '{Param_date_chargement}'
                {OPTIONAL_ARG_1}
                {OPTIONAL_ARG_2}
                AND T_CHARGEMENT.CODE_AGCE = {CODE_AGCE}
            GROUP BY 
        ...
        kwargs['OPTIONAL_ARG_1'] = '' if kwargs['Param_operateur'] in self.null_values else kwargs['OPTIONAL_ARG_1']
        kwargs['OPTIONAL_ARG_2'] = 'AND	T_COND_CHARGEE.CODE_COND = {Param_code_cp}'
        kwargs['OPTIONAL_ARG_2'] = '' if kwargs['Param_code_cp'] in self.null_values else kwargs['OPTIONAL_ARG_2']
    ...
    def Req_synthese_livraison_date(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND STATISTIQUES.dbo.T_SYNTHESE_LIVRAISON.DATE_JOURNEE = '{Param_DATE_JOURNEE}'
    ...
    def Req_tache_operateur(self, args):
        query = '''
            SELECT 
                T_OPERTEURS_TACHES.CODE_OPERATEUR AS CODE_OPERATEUR,	
                T_OPERTEURS_TACHES.ID_TACHE AS ID_TACHE
    ...
    def Req_total_autorisation_caisserie(self, args):
        query = '''
            SELECT 
                ...
                T_AUTORISATION_SOLDE_CAISSERIE.PAL_UHT AS PAL_UHT,	
                T_AUTORISATION_SOLDE_CAISSERIE.PAL_EURO AS PAL_EURO,
                T_AUTORISATION_SOLDE_CAISSERIE.CS1 AS CS_1,
                T_AUTORISATION_SOLDE_CAISSERIE.CS2 AS CS_2
            FROM 
                T_AUTORISATION_SOLDE_CAISSERIE
            WHERE 
    ...
    def Req_total_cmd_secteur(self, args):
        query = '''
            SELECT 
                T_COMMANDES.DATE_LIVRAISON AS DATE_LIVRAISON,	
                T_COMMANDES.code_secteur AS code_secteur,	
                ...
                T_ARTICLES_AGENCE.QTE_PACK AS QTE_PACK,	
                SUM(T_PRODUITS_COMMANDES.QTE_U) AS la_somme_QTE_U,	
                SUM(( T_PRODUITS_COMMANDES.QTE_U / T_ARTICLES_AGENCE.QTE_PACK ) ) AS la_somme_QTE_PACK,	
                SUM(T_PRODUITS_COMMANDES.QTE_C) AS la_somme_QTE_C
            FROM 
                T_COMMANDES,	
                T_PRODUITS_COMMANDES,	
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_COMMANDES.DATE_LIVRAISON = \'{Param_date_livraison}\''
        kwargs['OPTIONAL_ARG_2'] = 'AND	T_PRODUITS.CODE_FAMILLE IN ({param_lst_famille})'

        if kwargs['Param_date_livraison'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''

        if kwargs['param_lst_famille'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''
    ...
    def Req_total_commandes(self, args):
        query = '''
            SELECT 
                T_PRODUITS_COMMANDES.CODE_ARTICLE AS CODE_ARTICLE,	
                SUM(T_PRODUITS_COMMANDES.QTE_U) AS la_somme_QTE_U,	
    ...
    def Req_total_commandes_periode(self, args):
        query = '''
            SELECT 
                T_PRODUITS_COMMANDES.CODE_ARTICLE AS CODE_ARTICLE,	
                SUM(T_PRODUITS_COMMANDES.QTE_U) AS la_somme_QTE_U,	
            ...
            WHERE 
                T_COMMANDES.ID_COMMANDE = T_PRODUITS_COMMANDES.ID_COMMANDE
                AND T_COMMANDES.DATE_LIVRAISON BETWEEN '{Param_dt1}' AND '{Param_dt2}'
                AND T_COMMANDES.TYPE_COMMANDE <> 'U'
                AND T_COMMANDES.CODE_AGENCE = {CODE_AGCE}
            GROUP BY 
                T_PRODUITS_COMMANDES.CODE_ARTICLE
    ...
    def Req_total_cond_charg_supp_secteur(self, args):
        query = '''
            SELECT 
    ...
    def Req_total_cond_retour_secteur(self, args):
        query = '''
            ...
            WHERE 
                T_LIVRAISON.NUM_LIVRAISON = T_COND_LIVRAISON.NUM_LIVRAISON
                AND T_COND_LIVRAISON.code_secteur = {Param_code_secteur}
    ...
    def Req_total_credit_secteur(self, args):
        query = '''
            SELECT 
                T_PRODUITS_LIVREES.CODE_ARTICLE AS CODE_ARTICLE,	
                T_PRODUITS_LIVREES.code_secteur AS code_secteur,	
    ...
    def Req_total_decompte_espece(self, args):
        query = '''
            SELECT 
                T_DECOMPTE.CODE_OPERATEUR AS CODE_OPERATEUR,	
                T_DECOMPTE.DATE_VALIDATION AS DATE_VALIDATION,	
    ...
    def Req_total_depense_categorie(self, args):
        query = '''
            SELECT 
                T_OPERATIONS_CAISSE.TYPE_OPERATION AS TYPE_OPERATION,	
                T_MOUVEMENTS_CAISSE.CODE_CAISSE AS CODE_CAISSE,	
        ...
        try:
            kwargs = {
                'Param_type_operation': '\', \''.join(args[0].split(',')),
                'Param_code_caisse': args[1],
                'Param_date_validation': args[2],
                'CODE_AGCE': args[len(args) - 1]
            }
    ...
    def Req_total_dons_secteur(self, args):
        query = '''
            SELECT 
                T_PRODUITS_LIVREES.code_secteur AS code_secteur,	
                T_PRODUITS_LIVREES.CODE_ARTICLE AS CODE_ARTICLE,	
            ...
            WHERE 
                T_LIVRAISON.NUM_LIVRAISON = T_PRODUITS_LIVREES.NUM_LIVRAISON
                AND T_PRODUITS_LIVREES.TYPE_MVT = 'D'
                AND	T_PRODUITS_LIVREES.code_secteur = {Param_code_secteur}
                AND	T_PRODUITS_LIVREES.DATE_VALIDATION = '{Param_date_validation}'
    ...
    def Req_total_ecart_chargement(self, args):
        query = '''
            SELECT 
                T_COND_CHARGEE.DATE_CHARGEMENT AS DATE_CHARGEMENT,	
                T_COND_CHARGEE.COMPTE_ECART AS COMPTE_ECART,	
    ...
    def Req_total_ecart_cond(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_OPERATIONS.DATE_OPERATION = '{Param_date_mvt}'
                {OPTIONAL_ARG_1}
                {OPTIONAL_ARG_2}
                AND T_OPERATIONS.CODE_AGCE = {CODE_AGCE}
            GROUP BY 
        ...
        if kwargs['Param_operateur'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''

        return query.format(**kwargs).format(**kwargs)

    def Req_total_ecart_inventaires(self, args):
        query = '''
            SELECT 
                T_MOUVEMENTS.DATE_MVT AS DATE_MVT,	
                T_MOUVEMENTS.TYPE_MOUVEMENT AS TYPE_MOUVEMENT,	
    ...
    def Req_total_livraison(self, args):
        query = '''
            SELECT 
                T_LIVRAISON.DATE_LIVRAISON AS DATE_LIVRAISON,	
                T_PRODUITS_LIVREES.CODE_ARTICLE AS CODE_ARTICLE,	
    ...
    def Req_total_livraison_client(self, args):
        query = '''
            ...
            WHERE 
                T_LIVRAISON.NUM_LIVRAISON = T_PRODUITS_LIVREES.NUM_LIVRAISON
                AND T_LIVRAISON.DATE_VALIDATION BETWEEN '{Param_dt1}' AND '{Param_dt2}'
                AND	T_LIVRAISON.TYPE_MVT IN ('L', 'R') 
                AND	T_LIVRAISON.STATUT <> 'A'
                AND T_LIVRAISON.CODE_AGCE = {CODE_AGCE}
            GROUP BY 
    ...
    def Req_total_mvt_caisse(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_OPERATIONS_CAISSE.DATE_OPERATION = \'{Param_date_operation}\''
        if not kwargs['Param_date_operation']:
            kwargs['OPTIONAL_ARG_1'] = ''

        return query.format(**kwargs).format(**kwargs)

    def Req_total_mvt_chargement(self, args):
        query = '''
            SELECT 
                T_CHARGEMENT.DATE_CHARGEMENT AS DATE_CHARGEMENT,	
                T_PRODUITS_CHARGEE.CODE_ARTICLE AS CODE_ARTICLE,	
    ...
    def Req_total_mvt_cond(self, args):
        ...
        if kwargs['Param_sous_type'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''

        return query.format(**kwargs).format(**kwargs)

    def Req_total_mvt_cond_cac(self, args):
        query = '''
            SELECT 
                T_COND_LIVRAISON.CODE_CP AS CODE_CP,	
                T_COND_LIVRAISON.TYPE_CLIENT AS TYPE_CLIENT,	
    ...
    def Req_total_mvt_operation(self, args):
        ...
        if kwargs['Param_ls_motifs'] in self.null_values:
            kwargs['OPTIONAL_ARG_2'] = ''
    ...
    def Req_total_position_cond(self, args):
        query = '''
            ...
            FROM 
                ...
                T_MAGASINS a
            WHERE
                T_MAGASIN_COND.CODE_MAGASIN = a.CODE_MAGASIN
                AND a.CODE_AGCE = {CODE_AGCE}
            GROUP BY 
                T_MAGASIN_COND.CODE_CP
        '''
    ...
    def Req_total_prelev(self, args):
        query = '''
            ...
            WHERE 
                T_PRELEVEMENT_SUSP_COND.ID_PRELEV = T_DT_PRELEVEMENT_COND.ID_PRELEVEMENT
                AND T_PRELEVEMENT_SUSP_COND.DATE_VALIDATION <> '1900-01-01 00:00:00'
                AND T_PRELEVEMENT_SUSP_COND.CODE_AGCE = {CODE_AGCE}
            GROUP BY 
                T_DT_PRELEVEMENT_COND.CODE_OPERATEUR
        '''
    ...
    def Req_total_prelev_journalier(self, args):
        query = '''
            ...
            WHERE 
                T_PRELEVEMENT_SUSP_COND.ID_PRELEV = T_DT_PRELEVEMENT_COND.ID_PRELEVEMENT
                AND T_OPERATEUR.CODE_OPERATEUR = T_DT_PRELEVEMENT_COND.CODE_OPERATEUR
                AND T_PRELEVEMENT_SUSP_COND.DATE_VALIDATION = '{Param_date_validation}'
                AND T_PRELEVEMENT_SUSP_COND.CODE_AGCE = {CODE_AGCE}
            GROUP BY 
                T_OPERATEUR.FONCTION,	
                T_PRELEVEMENT_SUSP_COND.DATE_VALIDATION
    ...
    def Req_total_reconaissances(self, args):
        ...
        if kwargs['Param_date_max'] in self.null_values:
            return ValueError

        kwargs['OPTIONAL_ARG_1'] = 'AND T_RECONAISSANCES.DATE_RECONAISS = \'{Param_date_recon}\''
        if kwargs['Param_date_recon'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_total_reglement(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_DECOMPTE.REGLEMENT = {Param_type_reg}'
        if kwargs['Param_type_reg'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_total_regularisation_MS(self, args):
        ...
        if kwargs['Param_motif'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_total_retour_secteur(self, args):
        query = '''
            SELECT 
                T_PRODUITS_LIVREES.code_secteur AS code_secteur,	
                T_PRODUITS_LIVREES.DATE_VALIDATION AS DATE_VALIDATION,	
            ...
            WHERE 
                T_LIVRAISON.NUM_LIVRAISON = T_PRODUITS_LIVREES.NUM_LIVRAISON
                AND T_PRODUITS_LIVREES.TYPE_MVT = 'R'
                AND	T_PRODUITS_LIVREES.code_secteur = {Param_code_secteur}
                AND	T_PRODUITS_LIVREES.DATE_VALIDATION = '{Param_date_validation}'
                AND	T_LIVRAISON.STATUT <> 'A'
                AND T_LIVRAISON.CODE_AGCE = {CODE_AGCE}
    ...
    def Req_total_solde_init_clients(self, args):
        query = '''
            SELECT 
                T_SOLDE_INITIAL_CLIENT.DATE_JOURNEE AS DATE_JOURNEE,	
    ...
    def Req_total_sortie_rendus(self, args):
        query = '''
            SELECT 
                T_MOUVEMENTS.DATE_MVT AS DATE_MVT,	
                T_MOUVEMENTS.TYPE_MOUVEMENT AS TYPE_MOUVEMENT,	
    ...
    def Req_total_statistiques_canal(self, args):
        query = '''
            SELECT 
                STATISTIQUES.dbo.T_STATISTIQUES.CATEGORIE AS CATEGORIE,	
                T_ARTICLES.CODE_PRODUIT AS CODE_PRODUIT,	
    ...
    def Req_total_transfert_sortie(self, args):
        query = '''
            SELECT 
                T_MOUVEMENTS.DATE_MVT AS DATE_MVT,	
                T_MOUVEMENTS.TYPE_MOUVEMENT AS TYPE_MOUVEMENT,	
    ...
    def Req_total_transferts_produit(self, args):
        query = '''
            SELECT 
                T_MOUVEMENTS.DATE_MVT AS DATE_MVT,	
                T_MOUVEMENTS.TYPE_MOUVEMENT AS TYPE_MOUVEMENT,	
    ...
    def Req_upd_chargement_1(self, args):
        ...
        if kwargs['Param_CODE_PREVENDEUR'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''

        return query.format(**kwargs).format(**kwargs)

    def Req_upd_etat_liv(self, args):
        query = '''
            UPDATE 
                T_LIVRAISON
            SET
    ...
    def Req_valider_commande_usine(self, args):
        query = '''
            BEGIN
                SET NOCOUNT ON;
                BEGIN TRY
    ...
    def Req_upd_statut_remise(self, args):
        query = '''
            ...
            WHERE 
                T_REMISE_CLIENT_TRAD.Date_Debut = '{Param_DATE_DEBUT}'
                AND	T_REMISE_CLIENT_TRAD.STATUT = {Param_STATUT_ACT}
                AND T_REMISE_CLIENT_TRAD.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_upd_statut_remise_client(self, args):
        query = '''
            UPDATE 
                T_REMISE_CLIENT_TRAD
            SET
    ...
    def Req_update_cond(self, args):
        return self.Req_maj_position_cond(args)
    ...
    def Req_update_dispo(self, args):
        ...
        if kwargs['Param_dispo'] in self.null_values:
            return ValueError
    ...
    def Req_update_motif_non_livraison(self, args):
        query = '''
            UPDATE 
                T_LIVRAISON
            SET
    ...
    def Req_update_NS_CAISSERIE(self, args):
        ...
        try:
            kwargs = {
                ...
                'Param_NS_P_UHT': args[3],
                'Param_NS_C_AG': args[4],
                'Param_NS_C_PR': args[5],
                'Param_NS_EURO': args[6],
                'Param_NS_C_BLC': args[7],
                'Param_NV_CS1': args[8],
        ...
        for key in kwargs:
            if kwargs[key] in self.null_values and key != 'Param_solde_emballage':
                return ValueError

        kwargs['Param_date_journee'] = self.validateDate(kwargs['Param_date_journee'])
        kwargs['OPTIONAL_ARG_1'] = 'SOLDE_EMB = {Param_solde_emballage},'

        if kwargs['Param_solde_emballage'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_update_tx_couverture_article(self, args):
        query = '''
            UPDATE
                T_ARTICLES
            SET
    ...
    def Req_update_vendeur_cond(self, args):
        query = '''
            UPDATE 
                T_COND_CHARGEE
            SET
    ...
    def Req_val_objectif_secteur(self, args):
        ...
        kwargs['OPTIONAL_ARG_3'] = 'AND	T_PRODUITS.CAT_PRODUIT = {Param_cat_produit}'
        if kwargs['Param_cat_produit'] in self.null_values:
            kwargs['OPTIONAL_ARG_3'] = ''
    ...
    def req_valeur_chargement_secteur(self, args):
        ...
        kwargs['OPTIONAL_ARG_3'] = '' if kwargs['Param_code_produit'] in self.null_values else kwargs['OPTIONAL_ARG_3']
        kwargs['OPTIONAL_ARG_4'] = 'AND	T_PRODUITS.CODE_FAMILLE = {Param_code_famille}'
        kwargs['OPTIONAL_ARG_4'] = '' if kwargs['Param_code_famille'] in self.null_values else kwargs['OPTIONAL_ARG_4']

        return query.format(**kwargs).format(**kwargs)

    def Req_valeur_chargements(self, args):
        query = '''
            SELECT 
                T_CHARGEMENT.DATE_CHARGEMENT AS DATE_CHARGEMENT,	
                T_CHARGEMENT.code_secteur AS code_secteur,	
    ...
    def Req_valeur_commande(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = '' if kwargs['Param_type_commande'] in self.null_values else kwargs['OPTIONAL_ARG_1']
        kwargs['OPTIONAL_ARG_2'] = 'AND	T_PRODUITS.CODE_PRODUIT = {Param_code_produit}'
        kwargs['OPTIONAL_ARG_2'] = '' if kwargs['Param_code_produit'] in self.null_values else kwargs['OPTIONAL_ARG_2']
        kwargs['OPTIONAL_ARG_3'] = 'AND	T_PRODUITS.CODE_FAMILLE = {Param_code_famille}'
        kwargs['OPTIONAL_ARG_3'] = '' if kwargs['Param_code_famille'] in self.null_values else kwargs['OPTIONAL_ARG_3']
        kwargs['OPTIONAL_ARG_4'] = 'AND	T_PRODUITS.CAT_PRODUIT = {Param_cat_produit}'
        kwargs['OPTIONAL_ARG_4'] = '' if kwargs['Param_cat_produit'] in self.null_values else kwargs['OPTIONAL_ARG_4']
        kwargs['OPTIONAL_ARG_5'] = 'AND	T_COMMANDES.CODE_CLIENT = {Param_code_client}'
        kwargs['OPTIONAL_ARG_5'] = '' if kwargs['Param_code_client'] in self.null_values else kwargs['OPTIONAL_ARG_5']
    ...
    def Req_validation_cond_livraison(self, args):
        ...
        if kwargs['Param_qte_chargee'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_vent_n1_produit(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_MOY_VENTE_ARTICLE.DATE_VENTE = '{Param_date}'
    ...
    def Req_ventes_secteur(self, args):
        query = '''
            SELECT 
                T_CHARGEMENT.DATE_CHARGEMENT AS DATE_CHARGEMENT,	
                T_CHARGEMENT.code_secteur AS code_secteur,	
    ...
    def Req_verif_chargement(self, args):
        query = '''
            SELECT 
                T_CHARGEMENT.code_secteur AS code_secteur,	
                T_CHARGEMENT.HEURE_SORTIE AS HEURE_SORTIE,	
        ...
        kwargs['Param_date_chargement'] = self.validateDate(kwargs['Param_date_chargement'])
        kwargs['OPTIONAL_ARG_1'] = 'AND T_SECTEUR.NOM_SECTEUR = {Param_nom_secteur}'
        kwargs['OPTIONAL_ARG_2'] = 'AND T_CHARGEMENT.DATE_CHARGEMENT = \'{Param_date_chargement}\''

        if kwargs['OPTIONAL_ARG_1'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''
    ...
    def Req_verif_envoi_perte(self, args):
        ...
        kwargs['Param_date_operation'] = self.validateDate(kwargs['Param_date_operation'])
        kwargs['OPTIONAL_ARG_1'] = 'T_OPERATIONS.DATE_OPERATION = \'{Param_date_operation}\''
    ...
    def Req_verif_n1_secteurs(self, args):
        query = '''
            SELECT 
                T_MOY_VENTE_ARTICLE.DATE_VENTE AS DATE_VENTE,	
                T_SECTEUR.NOM_SECTEUR AS NOM_SECTEUR,	
            ...
            WHERE 
                ...
                AND T_ARTICLES.CODE_ARTICLE = T_PRIX.CODE_ARTICLE
                AND T_ARTICLES.CODE_ARTICLE = T_MOY_VENTE_ARTICLE.CODE_PRODUIT
                AND T_MOY_VENTE_ARTICLE.DATE_VENTE = '{Param_dt}'
                AND	T_PRIX.Date_Debut <= '{Param_dt}'
                AND	T_PRIX.Date_Fin >= '{Param_dt}'
                AND	T_PRIX.CODE_AGCE = {CODE_AGCE}
            GROUP BY 
    ...
    def Req_verif_prelev_caisserie(self, args):
        query = '''
            ...
            WHERE 
                T_PRELEVEMENT_SUSP_COND.ID_PRELEV = T_DT_PRELEVEMENT_COND.ID_PRELEVEMENT
                AND T_PRELEVEMENT_SUSP_COND.DATE_PRELEV = '{Param_date_prelev}'
                AND T_PRELEVEMENT_SUSP_COND.CODE_AGCE = {CODE_AGCE}
            GROUP BY 
    ...
    def Req_verif_prelevement(self, args):
        query = '''
            SELECT 
                T_PRELEVEMENT_SUSP_COND.DATE_PRELEV AS DATE_PRELEV,	
                SUM(T_DT_PRELEVEMENT_COND.SUSP_VENTE) AS la_somme_SUSP_VENTE
    ...
    def Req_verif_traitement(self, args):
        query = '''
            SELECT 
                T_HISTORIQUE_OPERATIONS.COMMENTAIRE
            FROM 
        ...
        if kwargs['Param_cmt'] in self.null_values:
            return ValueError
    ...
    def Req_versement_non_envoyer(self, args):
        query = '''
            ...
            WHERE 
                ...
                AND T_OPERATIONS_CAISSE.MOTIF_ENVOI <> 1
                AND T_OPERATIONS_CAISSE.CODE_AGCE = {CODE_AGCE}
            ORDER BY 
                CODE_OPERATION DESC
        '''
    ...
    def Req_zero_stock(self, args):
        query = '''
            ...
            INNER JOIN T_MAGASINS a
                ON m.MAGASIN = a.CODE_MAGASIN
            WHERE
    ...
    def Requ_mvt_cond_gms(self, args):
        query = '''
            ...
            WHERE 
                T_LIVRAISON.NUM_LIVRAISON = T_COND_LIVRAISON.NUM_LIVRAISON
                AND T_LIVRAISON.DATE_VALIDATION = '{Param_date_livraison}'
                AND	T_LIVRAISON.STATUT <> 'A'
                {OPTIONAL_ARG_1}
                AND	T_LIVRAISON.TYPE_MVT IN ('l', 'R')
                AND T_LIVRAISON.CODE_AGCE = {CODE_AGCE}
    ...
    def journee_1_Requete(self, args):
        query = '''
            SELECT TOP 10 
                T_JOURNEE.DATE_JOURNEE AS DATE_JOURNEE,	
                T_JOURNEE.CODE_AGCE AS CODE_AGCE,	
    ...
    def Req_fen_transfert(self, args):
        query = '''
            SELECT 
                T_OPERATEUR.CODE_OPERATEUR AS CODE_OPERATEUR,	
                T_OPERATEUR.NOM_OPERATEUR AS NOM_OPERATEUR,	
    ...
    def Req_ls_articles_prix(self, args):
        query = '''
            SELECT 
                T_ARTICLES.CODE_ARTICLE AS CODE_ARTICLE,	
                T_ARTICLES.LIBELLE AS LIBELLE,	
...
# --------------- Conversions ----------------------
    def Req_conv_login(self, args):
        ...
        kwargs['OPTIONAL_ARG_1'] = 'AND T_OPERATEUR.CODE_OPERATEUR = {code_op}'
        if kwargs['code_op'] in self.null_values:
            kwargs['OPTIONAL_ARG_1'] = ''

        return query.format(**kwargs).format(**kwargs)
    
    def Req_conv_parametres(self, args):
        query = '''
            SELECT
                T_PARAMETRES.ID_PARAM AS ID_PARAM,
    ...
    def Req_conv_generation_in_1(self, args):
        query = '''
            SELECT 
                T_ARTICLES.CODE_ARTICLE,
                T_ARTICLES.RANG,
                T_ARTICLES.ABREVIATION,
                T_ARTICLES.LIBELLE_COURT,
                T_ARTICLES_AGENCE.CONDITIONNEMENT,
                T_ARTICLES_AGENCE.QTE_PALETTE,
                T_ARTICLES_AGENCE.TYPE_CAISSE,
                T_ARTICLES_AGENCE.TYPE_PALETTE,
                T_ARTICLES.CODE_PRODUIT,
                T_ARTICLES_AGENCE.QTE_PACK,
                T_PRIX_AGENCE.PRIX_VENTE,
                T_ARTICLES.TVA,
                T_ARTICLES.AFF_REPARTITION,
                T_ARTICLES.DISPO
            FROM 
                T_ARTICLES,
                T_ARTICLES_AGENCE,
                T_PRIX_AGENCE 
            WHERE 
                T_PRIX_AGENCE.CODE_AGCE = {CODE_AGCE}
                AND T_ARTICLES.CODE_ARTICLE = T_PRIX_AGENCE.CODE_ARTICLE
                AND T_ARTICLES_AGENCE.CODE_AGCE = {CODE_AGCE}
                AND T_ARTICLES.CODE_ARTICLE = T_ARTICLES_AGENCE.CODE_ARTICLE
                AND T_ARTICLES_AGENCE.ACTIF = '1'
            ...
            select
                ...
                (
                    ...
                    from
                        T_TOURNEES
                    where
                        T_TOURNEES.CODE_SECTEUR = T_SECTEUR.CODE_SECTEUR
                        and T_TOURNEES.ACTIF=1
                 ) AS NB_TRN
            from
                T_SECTEUR,
                T_BLOC,
                T_ZONE
            where
                T_SECTEUR.CODE_BLOC = T_BLOC.CODE_BLOC
				AND T_BLOC.CODE_ZONE = T_ZONE.CODE_ZONE
				AND T_ZONE.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_conv_generation_in_4(self, args):
        query = '''
            SELECT ss.CODE_SOUS_SECTEUR, ss.NOM_SOUS_SECTEUR
            FROM T_SOUS_SECTEUR ss
            INNER JOIN T_SECTEUR s ON s.CODE_SECTEUR = ss.CODE_SOUS_SECTEUR
            INNER JOIN T_BLOC b ON b.CODE_BLOC = s.CODE_BLOC
            INNER JOIN T_ZONE z ON z.CODE_ZONE = b.CODE_ZONE
                AND z.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_conv_generation_in_7(self, args):
        query = '''
            select T_CLIENTS.CODE_CLIENT,NOM_CLIENT,CAT_CLIENT,CLASSE,TELEPHONE,CODE_SECTEUR,TYPE_PRESENTOIRE,(select COUNT(*)
            from T_ITINERAIRES,T_TOURNEES where CODE_CLIENT = T_CLIENTS.CODE_CLIENT and T_ITINERAIRES.CODE_TOURNEE=T_TOURNEES.CODE_TOURNEE and T_TOURNEES.ACTIF=1) AS FRQ,ADRESSE,SOUS_SECTEUR
    ...
    def Req_conv_generation_in_9(self, args):
        ...
        if kwargs['param_nbj'] in self.null_values:
            return ValueError

        return query.format(**kwargs)

    def Req_conv_affiche_ls_inventaire_nom_magasin(self, args):
        query = '''
            SELECT
                T_MAGASINS.NOM_MAGASIN AS NOM_MAGASIN
            FROM
        ...
        if kwargs['code_magasin'] in self.null_values:
            return ValueError
        
        return query.format(**kwargs)

    def Req_conv_affiche_ls_inventaire_nom_operateur(self, args):
        query = '''
            SELECT
                T_OPERATEUR.NOM_OPERATEUR AS NOM_OPERATEUR
            FROM
        ...
        if kwargs['code_operateur'] in self.null_values:
            return ValueError
        
        return query.format(**kwargs)

    def Req_conv_fen_inventaire_btn_supprimer_compte_ecart(self, args):
        query = '''
            SELECT
                T_MOUVEMENTS_CAISSERIE.COMPTE_ECART AS COMPTE_ECART
            FROM
        ...
        if kwargs['id_mouvement'] in self.null_values:
            return ValueError
        
        return query.format(**kwargs)

    def Req_conv_fen_inventaire_btn_supprimer_compte_ecart2(self, args):
        query = '''
            SELECT
                T_MOUVEMENTS.COMPTE_ECART AS COMPTE_ECART
            FROM
    ...
    def Req_conv_fen_inventaire_btn_supprimer_code_operation(self, args):
        query = '''
            SELECT
                T_OPERATIONS.CODE_OPERATION AS CODE_OPERATION
            FROM
    ...
    def Req_conv_journee_btn_nouv_journee_date_journee(self, args):
        query = '''
            SELECT DISTINCT
                T_JOURNEE.DATE_JOURNEE AS DATE_JOURNEE
            FROM
    ...
    def Req_conv_journee_btn_nouv_journee_articles_magasins(self, args):
        query = '''
            SELECT
                *
            FROM
                ...
                T_MAGASINS
            WHERE
                T_ARTICLES_MAGASINS.MAGASIN = T_MAGASINS.CODE_MAGASIN
                AND T_MAGASINS.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_conv_journee_btn_nouv_journee_stock_init(self, args):
        query = '''
            INSERT INTO 
        ...
        if kwargs['date_ps'] in self.null_values:
            return ValueError
        
        return query.format(**kwargs)

    def Req_conv_journee_btn_nouv_journee_magasin_cond(self, args):
        return self.Req_select_magasin_cond(args)

    def Req_conv_journee_btn_nouv_journee_stock_initi_cond(self, args):
        query = '''
            INSERT INTO 
        ...
        if kwargs['date_journee'] in self.null_values:
            return ValueError
        
        return query.format(**kwargs)

    def Req_conv_journee_btn_nouv_journee_preparation_chargements(self, args):
        query = '''
            DELETE FROM
                T_PREPARATION_CHARGEMENTS
            WHERE
                T_PREPARATION_CHARGEMENTS.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_conv_journee_btn_nouv_journee_journee(self, args):
        query = '''
            INSERT INTO 
                T_JOURNEE
    ...
    def Req_conv_journee_btn_cloturer_livraision_planning(self, args):
        query = '''
            SELECT
            ...
            WHERE
                T_LIVRAISON_PLANNING.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_conv_journee_btn_cloturer_synthese_livraision(self, args):
        query = '''
            INSERT INTO 
                STATISTIQUES.dbo.T_SYNTHESE_LIVRAISON 
    ...
    def Req_conv_journee_btn_cloturer_creer_journal(self, args):
        query = '''
            INSERT INTO 
                T_HISTORIQUE_OPERATIONS 
        ...
        if kwargs['date_heure'] in self.null_values:
            return ValueError
        
        return query.format(**kwargs)

    def Req_conv_journee_btn_cloturer_creer_synchro(self, args):
        query = '''
            INSERT INTO 
                T_SYNCHRO 
        ...
        if kwargs['dh_creation'] in self.null_values:
            return ValueError
        
        return query.format(**kwargs)

    def Req_conv_journee_btn_cloturer_journee_temp(self, args):
        query = '''
            UPDATE 
                T_JOURNEE
            SET
    ...
    def Req_conv_journee_btn_cloturer_solde_initial_caisse(self, args):
        query = '''
            INSERT INTO 
                T_SOLDE_INITIAL_CAISSE 
    ...
    def Req_conv_journee_btn_cloturer_controle_cloture(self, args):
        query = '''
            select A.code_op,COUNT(A.code_op) as nb from (
            select DATE_CHARGEMENT,CODE_VENDEUR as CODE_OP from T_CHARGEMENT where CODE_VENDEUR<>{CODE_0}
                AND T_CHARGEMENT.CODE_AGCE = {CODE_AGCE}
            union all
            select DATE_CHARGEMENT,CODE_CHAUFFEUR as CODE_OP from T_CHARGEMENT where CODE_CHAUFFEUR<>{CODE_0}
                AND T_CHARGEMENT.CODE_AGCE = {CODE_AGCE}
            union all
            select DATE_CHARGEMENT,AIDE_VENDEUR1 as CODE_OP from T_CHARGEMENT where AIDE_VENDEUR1<>{CODE_0}
                AND T_CHARGEMENT.CODE_AGCE = {CODE_AGCE}
            union all
            select DATE_CHARGEMENT,AIDE_VENDEUR2 as CODE_OP from T_CHARGEMENT where AIDE_VENDEUR2<>{CODE_0}
                AND T_CHARGEMENT.CODE_AGCE = {CODE_AGCE}
    ...
    def Req_conv_journee_btn_import_moy_vente_article(self, args):
        query = '''
            INSERT INTO 
                T_MOY_VENTE_ARTICLE 
        ...
        if kwargs['date_vente'] in self.null_values:
            return ValueError
        
        return query.format(**kwargs)

    def Req_conv_journee_btn_import_moy_vente_clients(self, args):
        query = '''
            INSERT INTO 
                T_MOY_VENTE_ARTICLE 
    ...
    def Req_conv_codification_operation_cod_std(self, args):
        query = '''
            SELECT
    ...
    def Req_conv_fen_inventaire_btn_appliquer_operations(self, args):
        query = '''
            INSERT INTO 
                T_OPERATIONS
        ...
        if (kwargs['date_operation'] in self.null_values
              or kwargs['date_heure'] in self.null_values):
            return ValueError
        
        return query.format(**kwargs)

    def Req_conv_fen_inventaire_btn_appliquer_mouvements_caisserie(self, args):
        query = '''
            INSERT INTO 
                T_MOUVEMENTS_CAISSERIE
    ...
    def Req_conv_fen_inventaire_btn_appliquer_mouvements(self, args):
        query = '''
            INSERT INTO 
                T_MOUVEMENTS
        ...
        if kwargs['date_mvt'] in self.null_values:
            return ValueError

        return query.format(**kwargs)

    def Req_conv_maj_position_articles_magasins(self, args):
        query = '''
            INSERT INTO 
                T_ARTICLES_MAGASINS 
    ...
    """def Req_conv_maj_position_articles_magasins_qte_stock(self, args):
        query = '''
            UPDATE 
                T_ARTICLES_MAGASINS
            SET
    ...
    def Req_conv_codification_mvt_cond_cod_long(self, args):
        query = '''
            SELECT
    ...
    def Req_conv_clients_codification_client(self, args):
        query = '''
            SELECT 
                MAX(T_CLIENTS.CODE_CLIENT) AS maxn
            FROM 
                T_CLIENTS
            WHERE 
    ...
    def Req_conv_operateurs_btn_appliquer(self, args):
        query = '''
            DELETE FROM
                T_OPERTEURS_TACHES
            WHERE
                ID_TACHE in (select ID_TACHE from T_TACHES where APP='4')
                AND CODE_OPERATEUR = {CODE_OPERATEUR}
        '''
    ...
    def Req_conv_gestion_acces_btn_enregistrer(self, args):
        query = '''
            delete from T_OPERTEURS_TACHES
            where ID_TACHE in (select ID_TACHE from T_TACHES)
            and CODE_OPERATEUR= {CODE_OPERATEUR}
        '''
    ...
    def Req_select_magasin_cond(self, args):
        query = '''
            SELECT 
                mc.*
            FROM 
                T_MAGASIN_COND mc
                INNER JOIN T_MAGASINS m
                ON m.CODE_MAGASIN = mc.CODE_MAGASIN
            WHERE
                m.CODE_AGCE = {CODE_AGCE}
        '''
    ...
    def Req_conv_affectation_imprimer(self, args):
        query = '''
            select DATE_CHARGEMENT,NOM_SECTEUR,v.MATRICULE AS MAT_VENDEUR,V.NOM_OPERATEUR AS VENDEUR,c.MATRICULE AS MAT_CHAUFFEUR,ISNULL(C.NOM_OPERATEUR,'') AS CHAUFFEUR,A1.MATRICULE AS MAT_AIDE1,ISNULL(A1.NOM_OPERATEUR,'') AS AIDE1,A2.MATRICULE AS MAT_AIDE2,ISNULL(A2.NOM_OPERATEUR,'') AS AIDE2 
            from t_chargement,T_SECTEUR,T_OPERATEUR V,T_OPERATEUR c,T_OPERATEUR A1,T_OPERATEUR A2
            where T_CHARGEMENT.CODE_CHARGEMENT in (select distinct CODE_CHARGEMENT from T_PRODUITS_CHARGEE where DATE_CHARGEMENT between '{Param_dt1}' and '{Param_dt2}' and QTE_CHARGEE<>0 )
            and T_CHARGEMENT.CODE_SECTEUR = T_SECTEUR.CODE_SECTEUR
            AND T_CHARGEMENT.CODE_AGCE = {CODE_AGCE}
            and V.CODE_OPERATEUR = T_CHARGEMENT.CODE_VENDEUR
            and C.CODE_OPERATEUR = T_CHARGEMENT.CODE_CHAUFFEUR
            and A1.CODE_OPERATEUR = T_CHARGEMENT.AIDE_VENDEUR1
            and A2.CODE_OPERATEUR = T_CHARGEMENT.AIDE_VENDEUR2
            order by DATE_CHARGEMENT,RANG
        '''
    ...
    def Req_conv_pop_client_acredit(self, args):
        query = '''
            ...
            T.CLIENT_EN_COMPTE,T.SOLDE_C_STD
            FROM SIEGE.dbo.TMPS_CLIENTS T ,dbo.T_AGENCE A,dbo.T_GROUP_CLIENT G,dbo.T_CAT_CLIENTS E,dbo.T_SECTEUR S,dbo.T_SOUS_SECTEUR SS
            WHERE T.CODE_AGCE=A.CODE_AGCE AND G.ID_GROUP=T.GROUP_CLIENT AND E.CODE_CAT_CLIENT=T.CAT_CLIENT
            AND T.SOUS_SECTEUR=SS.CODE_SOUS_SECTEUR AND S.CODE_SECTEUR=SS.CODE_SECTEUR AND   A.CODE_AGCE={CODE_AGCE}  AND T.VALIDER_SIEGE=1  AND T.VALIDER_AGENCE IS NULL
        '''
    ...
    def Req_conv_nb_produits_commande(self, args):
        ...
        if kwargs['id'] in self.null_values:
            return ValueError
        ...
        query = '''
             ...
             WHERE 
                ...
                AND CODE_CLIENT IN (
                    SELECT code_client 
                    FROM T_LIVRAISON
                    WHERE DATE_LIVRAISON='{param_dt}'
                        AND CODE_AGCE = {CODE_AGCE})
        '''

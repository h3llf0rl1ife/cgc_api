...
BEGIN
	...
	BEGIN TRY
		...
		WHERE 
			(c.CODE_CLIENT NOT IN (SELECT CODE_CLIENT FROM COMMERCIAL.dbo.T_CLIENTS) AND c.ACTIF = 1)
			OR c.CODE_CLIENT = 0
...
*/
		...
		FROM 
			...
			INNER JOIN COMMERCIAL.dbo.T_PROMOTIONS p ON p.ID_PROMO = cp.ID_PROMO
		--WHERE 
		...
		SET
			c.rang = a.rang
		FROM
			commercial_t.dbo.t_articles AS c
			INNER JOIN agence_taroudant.dbo.t_articles AS a
